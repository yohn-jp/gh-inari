...
function addDiagnostic(diagnostics, code, message, structured = undefined) {
  const safeMessage = sanitizeCertificationText(String(message), MAX_CERTIFICATION_DIAGNOSTIC_MESSAGE_LENGTH);
  appendCertificationDiagnostic(
    diagnostics,
    code,
    safeMessage ?? "Inari command failed without safe diagnostics.",
    structured,
  );
}
...
function runInari(options, args, evidence) {
  ...
  if (!result.ok) {
    ...
    if (structuredFailure !== undefined) {
      ...
      throw new Error(`${args.join(" ")} failed: ${structuredFailure.message}`);
    }
    const detail = (result.error?.message ?? result.stderr.trim()) || `exit status ${result.status}`;
    const safeDetail =
      sanitizeCertificationText(detail, MAX_CERTIFICATION_DIAGNOSTIC_MESSAGE_LENGTH) ?? `exit status ${result.status}`;
...
function runWorker(options, handoff, sourceSha) {
  ...
  try {
    ...
    if (!result.ok) {
      const detail = (result.error?.message ?? result.stderr.trim()) || `exit status ${result.status}`;
      const safeDetail =
        sanitizeCertificationText(detail, MAX_CERTIFICATION_DIAGNOSTIC_MESSAGE_LENGTH) ??
        `exit status ${result.status}`;
      throw new Error(`implementation worker failed: ${safeDetail}`);
    }
  } finally {
