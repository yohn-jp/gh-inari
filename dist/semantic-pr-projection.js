...
function validateDesiredRelations(input, path, violations) {
    ...
    if (!isRecord(implementsRelation)) {
        addViolation(violations, "MUTATION_PLAN_INVALID", `${path}.implements`, "Desired implements relation must be an object.");
        return;
    }
    unknownProperties(implementsRelation, DESIRED_IMPLEMENTS_RELATION_KEYS, `${path}.implements`, violations, "MUTATION_PLAN_INVALID");
    if (implementsRelation.relation !== "implements")
        addViolation(violations, "MUTATION_PLAN_INVALID", `${path}.implements.relation`, 'Relation kind must be "implements".');
    if (!RELATION_REPRESENTATIONS.has(implementsRelation.representation))
        addViolation(violations, "MUTATION_PLAN_INVALID", `${path}.implements.representation`, "Relation representation is invalid.");
    if (!Array.isArray(implementsRelation.references)) {
        addViolation(violations, "MUTATION_PLAN_INVALID", `${path}.implements.references`, "Relation references must be an array.");
    }
    else {
        implementsRelation.references.forEach((entry, index) => {
            ...
            if (!result.valid)
                addViolation(violations, "MUTATION_PLAN_INVALID", `${path}.implements.references[${index}]`, "IssueReference is invalid.");
        });
    }
}
    ...
    input.forEach((entry, index) => {
        ...
        if (kind === "GOVERNANCE_GENERATION_MATCH") {
            ...
            if (head !== undefined && typeof desiredHead === "string" && head !== desiredHead)
                addViolation(violations, "MUTATION_PLAN_INVALID", `${entryPath}.head`, "Precondition head must equal desired head.");
            if (base !== undefined && typeof desiredBase === "string" && base !== desiredBase)
                addViolation(violations, "MUTATION_PLAN_INVALID", `${entryPath}.base`, "Precondition base must equal desired base.");
        }
    });
