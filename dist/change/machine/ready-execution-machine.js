...
const applyReadyEffect = fromPromise(({ input }) => input.services.apply(input.effect));
const DEFAULT_READ_FAILURE = {
    code: "CHANGE_EXECUTION_READ_FAILED",
    message: "Trusted Change evidence read failed closed.",
    diagnostics: [],
};
function defaultVerificationFailure(message = "Post-effect Ready projection verification failed.") {
    return {
...
function rereadFailure(context, failure) {
...
function lifecyclePreconditionFailure(state) {
    ...
    states: {
        ...
        projecting: {
            entry: assign(({ context }) => {
                ...
                if (input === undefined)
                    return { failure: DEFAULT_READ_FAILURE, projection: undefined };
                try {
            ...
            always: [
                {
                    target: "validating",
                    guard: ({ context }) => context.readStage === "initial" && context.projection !== undefined && context.failure === undefined,
                },
                {
                    target: "effectFailedResult",
                    guard: ({ context }) => context.readStage === "reread" &&
        ...
        validating: {
            entry: assign(({ context }) => {
                ...
                try {
                    ...
                    const validationInput = context.services.semantics.validationInput(input, projection.change, context.services.request.requester);
        ...
        planning: {
            entry: assign(({ context }) => {
                if (context.validationInput === undefined)
                    return { plan: undefined, effect: undefined };
                try {
        ...
        markingReady: {
            invoke: {
                ...
                onError: {
                    ...
                    actions: assign({
                        rereadMode: "effect-failed",
                        effectFailure: ({ context }) => context.effect === undefined ? undefined : context.services.failureForEffect(context.effect),
                    }),
                },
            },
        },
                ...
                onDone: [
                    {
                        ...
                        actions: assign({
                            input: ({ event }) => (event.output.ok ? event.output.input : undefined),
                            readStage: "reread",
                            failure: ({ context, event }) => event.output.ok ? undefined : rereadFailure(context, event.output.failure),
                            projection: undefined,
                        }),
                    },
                    {
                        target: "readFailed",
                        actions: assign({
                            failure: ({ context, event }) => event.output.ok ? DEFAULT_READ_FAILURE : rereadFailure(context, event.output.failure),
                        }),
                    },
                ],
                onError: {
                    target: "readFailed",
                    actions: assign({
                        failure: ({ context }) => context.rereadMode === "effect-failed"
                            ? {
                                code: "CHANGE_EXECUTION_READ_FAILED",
                                message: "Trusted Change evidence read failed after the Ready effect failed.",
                                diagnostics: [],
        ...
        verifying: {
            entry: assign(({ context }) => {
                ...
                try {
                    ...
                    const effect = plan.effects[0];
                    return {
                        result: effect?.kind === "MARK_PULL_REQUEST_READY"
        ...
        succeeded: {
            type: "final",
            entry: assign({
                outcome: ({ context }) => context.result === undefined
...
export async function executeReadyWithXState(services) {
...
//# sourceMappingURL=ready-execution-machine.js.map