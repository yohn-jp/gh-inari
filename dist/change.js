...
export function deriveCanonicalBranchIdentity(input) {
    ...
    if (governance !== undefined) {
        ...
        if (!classification.valid) {
            ...
            addDiagnostic(diagnostics, violation?.code === "BRANCH_POLICY_INVALID"
                ? "CHANGE_INVALID_BRANCH_GOVERNANCE"
                : "CHANGE_BRANCH_GOVERNANCE_MISMATCH", violation?.code === "BRANCH_POLICY_INVALID" ? "$.branchGovernance" : "$.branchGovernance.pattern", violation?.message ?? `Derived branch "${branch}" does not satisfy the repository's branch governance.`);
            return { valid: false, diagnostics: createChangeDiagnosticReport(diagnostics).diagnostics };
        }
    }
