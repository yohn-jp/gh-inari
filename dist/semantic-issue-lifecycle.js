...
export const SEMANTIC_ISSUE_LIFECYCLE_VERSION = "1";
...
function addDiagnostic(diagnostics, code, path, message, expected, actual) {
    if (diagnostics.length >= SEMANTIC_ISSUE_LIFECYCLE_LIMITS.diagnostics)
        return;
    diagnostics.push({
        code,
        path,
...
function unknownProperties(input, allowed, path, diagnostics) {
...
function normalizeSingleReference(value, path, diagnostics) {
...
function observedRelation(observed, kind, path, diagnostics) {
    ...
    if (typeof representation !== "string" || !REPRESENTATIONS.has(representation)) {
        addDiagnostic(diagnostics, "OBSERVED_ISSUE_INVALID", `${path}.relations.${kind}.representation`, "Relation representation is invalid.");
        return { status: "unavailable", references: [] };
    }
    if (representation === "conflict") {
        addDiagnostic(diagnostics, "RELATION_CONFLICT", `${path}.relations.${kind}`, "Conflicting relation evidence cannot be used for lifecycle derivation.");
        return { status: "unavailable", references: [] };
    }
    if (kind === "parent") {
        if (!hasOwn(relation, "reference"))
            return { status: "empty", references: [] };
        if (representation === "none") {
            addDiagnostic(diagnostics, "OBSERVED_ISSUE_INVALID", `${path}.relations.parent.reference`, "A parent reference cannot use the none representation.");
            return { status: "unavailable", references: [] };
        }
    ...
    if (!Array.isArray(relation.references)) {
        addDiagnostic(diagnostics, "OBSERVED_ISSUE_INVALID", `${path}.relations.dependsOn.references`, "dependsOn references must be an array.");
        return { status: "unavailable", references: [] };
    }
...
function validateObserved(input, path, diagnostics) {
    if (!isRecord(input)) {
...
function validateDeclaration(input, path, diagnostics) {
    if (!isRecord(input)) {
...
function validateChecklist(input, path, diagnostics) {
    if (!isRecord(input)) {
    ...
    if (input.status !== "present" && input.status !== "unavailable") {
        addDiagnostic(diagnostics, "CHECKLIST_INVALID", `${path}.status`, "Checklist status must be present or unavailable.");
        return undefined;
    }
    ...
    if (remaining.some((reference) => completedKeys.has(issueReferenceKey(reference))))
        addDiagnostic(diagnostics, "CHECKLIST_INVALID", path, "Checklist references cannot be both completed and remaining.");
    return { status: input.status, completed, remaining };
}
function declarationFromArtifact(input, path, diagnostics) {
    if (!isRecord(input)) {
    ...
    if (input.kind !== "issue" || input.version !== "1" || !isRecord(input.values)) {
        addDiagnostic(diagnostics, "DECLARATION_INVALID", path, "Lifecycle declarations require a materialized Issue Semantic Artifact.");
        return undefined;
    }
    const values = input.values;
    const declaration = {};
    for (const key of ["role", "supersedes", "supersededBy"])
        if (hasOwn(values, key))
            declaration[key] = values[key];
...
function normalizeInput(input, diagnostics) {
    ...
    input.issues.forEach((entry, index) => {
        ...
        seen.add(key);
        const observed = entry.observed === undefined ? undefined : validateObserved(entry.observed, `${path}.observed`, diagnostics);
        if (observed === undefined)
            addDiagnostic(diagnostics, "EVIDENCE_UNAVAILABLE", `${path}.observed`, "Observed Issue evidence is required to derive lifecycle relations.");
        const artifactDeclaration = entry.artifact === undefined
            ? undefined
            : declarationFromArtifact(entry.artifact, `${path}.artifact`, diagnostics);
        const directDeclaration = entry.declaration === undefined
            ? undefined
        ...
        if (artifactDeclaration !== undefined && directDeclaration !== undefined) {
            addDiagnostic(diagnostics, "DECLARATION_INVALID", path, "Supply lifecycle declaration through the Semantic Artifact or the adapter field, not both.");
        }
        const declaration = artifactDeclaration ?? directDeclaration;
        const checklist = entry.checklist === undefined ? undefined : validateChecklist(entry.checklist, `${path}.checklist`, diagnostics);
        const parent = observed === undefined
            ? { status: "unavailable", references: [] }
            : observedRelation(observed, "parent", `${path}.observed`, diagnostics);
        const dependsOn = observed === undefined
            ? { status: "unavailable", references: [] }
            : observedRelation(observed, "dependsOn", `${path}.observed`, diagnostics);
        if (observed?.number !== undefined && observed.number !== reference.number)
            addDiagnostic(diagnostics, "OBSERVED_ISSUE_INVALID", `${path}.observed.number`, "Observed Issue number must match its stable IssueReference.");
...
function addToMap(map, key, reference) {
...
function freezeReferenceMap(map) {
    const frozen = new Map();
    for (const [key, values] of map) {
        const unique = [...new Map(values.map((reference) => [issueReferenceKey(reference), reference])).values()].sort(compareReferences);
        frozen.set(key, Object.freeze(unique));
    }
    return frozen;
}
function deriveRelations(nodes, scope) {
    const children = new Map();
    const blocks = new Map();
    const supersedesMap = new Map();
    const supersededBy = new Map();
    const explicitSupersededBy = new Map();
    ...
    for (const node of nodes) {
        ...
        if (scope === "complete") {
            for (const reference of supersedesReferences)
                addToMap(supersedesMap, key, reference);
            for (const reference of supersedesReferences)
    ...
    if (scope === "complete") {
        for (const node of nodes) {
            if (node.parent.status === "present")
                for (const parent of node.parent.references)
                    addToMap(children, issueReferenceKey(parent), node.reference);
...
function lifecycleResult(diagnostics, projection) {
    return {
...
export function tryProjectSemanticIssueLifecycle(input) {
    const diagnostics = [];
    ...
    for (const node of normalized.nodes) {
        ...
        const supersedes = normalized.scope === "unavailable" ? (declaration?.supersedes ?? []) : (derived.supersedes.get(key) ?? []);
        const supersededByEffective = [
            ...new Map([...supersededBy, ...explicitInverse].map((reference) => [issueReferenceKey(reference), reference])).values(),
        ].sort(compareReferences);
        const issueDiagnostics = [];
        if (declaration?.supersededBy !== undefined && !sameReferences(explicitInverse, supersededBy))
            addDiagnostic(issueDiagnostics, "SUPERSESSION_DRIFT", `$.issues[${normalized.nodes.indexOf(node)}].declaration.supersededBy`, "Explicit supersededBy evidence disagrees with the derived supersession inverse.", supersededBy, explicitInverse);
        const childrenEvidence = normalized.scope === "unavailable" ? "unavailable" : children.length === 0 ? "empty" : "present";
        const blocksEvidence = normalized.scope === "unavailable" ? "unavailable" : blocks.length === 0 ? "empty" : "present";
        const parentEvidence = node.parent.status;
        const dependsOnEvidence = node.dependsOn.status;
        const role = declaration?.role;
        const completed = [];
        const remaining = [];
        let completionStatus = "not-declared";
        if (role === "tracker") {
            if (childrenEvidence === "unavailable") {
                completionStatus = "unknown";
                addDiagnostic(issueDiagnostics, "EVIDENCE_UNAVAILABLE", `$.issues[${normalized.nodes.indexOf(node)}].children`, "Complete child relation evidence is unavailable.");
            }
            else if (children.length === 0) {
                completionStatus = "unknown";
                addDiagnostic(issueDiagnostics, "EVIDENCE_UNAVAILABLE", `$.issues[${normalized.nodes.indexOf(node)}].children`, "A tracker requires at least one observed child to derive completion.");
            }
            else {
                for (const child of children) {
                    const childNode = normalized.nodes.find((candidate) => issueReferenceKey(candidate.reference) === issueReferenceKey(child));
                    const childState = childNode === undefined ? undefined : stateFor(childNode);
                    if (childState === "closed")
                        completed.push(child);
                    else if (childState === "open")
                        remaining.push(child);
                    else
                        addDiagnostic(issueDiagnostics, "EVIDENCE_UNAVAILABLE", `$.issues[${normalized.nodes.indexOf(node)}].children`, "A child Issue state is unavailable for completion derivation.");
                }
                if (completed.length + remaining.length !== children.length)
                    completionStatus = "unknown";
                else
                    completionStatus = remaining.length === 0 ? "complete" : "in-progress";
            }
        }
        else if (role === "leaf") {
            ...
            if (state === undefined) {
                completionStatus = "unknown";
                addDiagnostic(issueDiagnostics, "EVIDENCE_UNAVAILABLE", `$.issues[${normalized.nodes.indexOf(node)}].observed.state`, "Leaf completion requires observed Issue state.");
            }
            else
                completionStatus = state === "closed" ? "complete" : "in-progress";
            if (children.length > 0)
                addDiagnostic(issueDiagnostics, "ROLE_RELATION_DRIFT", `$.issues[${normalized.nodes.indexOf(node)}].declaration.role`, "A leaf Issue cannot have derived children.", [], children);
        }
        if (node.checklist?.status === "present" && role === "tracker" && completionStatus !== "unknown") {
            if (!sameReferences(node.checklist.completed, completed) || !sameReferences(node.checklist.remaining, remaining))
                addDiagnostic(issueDiagnostics, "CHECKLIST_RELATION_DRIFT", `$.issues[${normalized.nodes.indexOf(node)}].checklist`, "Checklist completion disagrees with authoritative child state.", { completed, remaining }, node.checklist);
        }
        else if (node.checklist?.status === "unavailable") {
            addDiagnostic(issueDiagnostics, "EVIDENCE_UNAVAILABLE", `$.issues[${normalized.nodes.indexOf(node)}].checklist`, "Checklist evidence is unavailable.");
        }
        if (node.observed?.state === "closed" && completionStatus === "in-progress")
            addDiagnostic(issueDiagnostics, "COMPLETION_DRIFT", `$.issues[${normalized.nodes.indexOf(node)}].observed.state`, "Issue is closed while authoritative child state still has a remainder.", "open", "closed");
        for (const diagnostic of issueDiagnostics)
            addDiagnostic(diagnostics, diagnostic.code, diagnostic.path, diagnostic.message, diagnostic.expected, diagnostic.actual);
        issues.push({
            ...
            supersessionEvidence: normalized.scope === "unavailable" ? "unavailable" : supersededByEffective.length === 0 ? "empty" : "present",
            completion: {
...
export function projectSemanticIssueLifecycle(input) {
    const result = tryProjectSemanticIssueLifecycle(input);
    if (!result.valid || result.projection === undefined)
        throw new SemanticIssueLifecycleError(result.diagnostics);
...
//# sourceMappingURL=semantic-issue-lifecycle.js.map