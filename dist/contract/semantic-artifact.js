...
import { issueReferenceKey, normalizeIssueReference } from "./issue-reference.js";
export const SEMANTIC_ARTIFACT_VERSION = "1";
...
function stableSerialize(value, stack = new WeakSet()) {
    ...
    if (stack.has(value))
        throw new TypeError("Cyclic JSON data.");
    stack.add(value);
    let result;
    if (Array.isArray(value)) {
...
function invalidEffectiveContract(message, path = "$") {
    return {
...
function declarationEntries(effective) {
    const entries = [];
    ...
    for (const field of effective.contract.fields ?? [])
        entries.push([field.id, field]);
    return entries;
}
/**
 ...
 */
function validateEffectiveContract(input) {
    if (!isRecord(input))
        return invalidEffectiveContract("Effective Artifact Contract must be an object.");
    if (input.version !== "1")
    ...
    try {
        ...
        const fieldsMatch = (Object.hasOwn(input, "fields") ? input.fields : undefined) === undefined
            ? !Object.hasOwn(expected, "fields")
            : stableSerialize(input.fields) === stableSerialize(expected.fields);
        const metadataMatches = input.version === expected.version &&
        ...
        for (const [name, declaration] of entries) {
            if (names.has(name))
                return invalidEffectiveContract(`Duplicate semantic declaration "${name}".`, `$.${name}`);
            names.add(name);
            if (!isRecord(declaration))
                return invalidEffectiveContract(`Declaration "${name}" is invalid.`, `$.${name}`);
            ...
            else if (declaration.authority === undefined ||
        ...
        for (const derivation of effective.derivations) {
            if (!isRecord(derivation) || typeof derivation.target !== "string" || derivations.has(derivation.target)) {
                return invalidEffectiveContract("Effective Contract derivations contain a duplicate or invalid target.", "$.derivations");
            }
            ...
            if (declaration === undefined || declaration.presence === "unused" || declaration.authority.kind !== "derived") {
                return invalidEffectiveContract(`Derivation target "${derivation.target}" is not a derived declaration.`, "$.derivations");
            }
            if (stableSerialize(derivation.operation) !== stableSerialize(declaration.authority.derive)) {
                return invalidEffectiveContract(`Derivation "${derivation.target}" does not match its declaration.`, "$.derivations");
            }
            ...
            if (derivation.operation.op === "format") {
                if (!Array.isArray(derivation.formatParts)) {
                    return invalidEffectiveContract(`Format derivation "${derivation.target}" is missing format parts.`, "$.derivations");
                }
                const references = derivation.formatParts
                    .filter((part) => isRecord(part) && part.kind === "reference" && isRecord(part.reference))
                    .map((part) => part.reference);
                if (stableSerialize(references) !== stableSerialize(derivation.dependencies)) {
                    return invalidEffectiveContract(`Format derivation "${derivation.target}" metadata is inconsistent.`, "$.derivations");
                }
            }
            else if (Object.hasOwn(derivation, "formatParts")) {
                return invalidEffectiveContract(`Non-format derivation "${derivation.target}" has format parts.`, "$.derivations");
            }
        }
        for (const [name, declaration] of entries) {
        ...
        if (!Array.isArray(effective.evaluationOrder) ||
        ...
        for (const [name, derivation] of derivations) {
            for (const dependency of derivation.dependencies) {
                ...
                if (dependencyPosition !== undefined && dependencyPosition >= positions.get(name)) {
                    return invalidEffectiveContract("Effective Contract evaluation order violates dependency order.", "$.evaluationOrder");
                }
            }
        ...
        const reparsed = parseArtifactContract(JSON.parse(serializeArtifactContract(effective.contract)));
        if (stableSerialize(reparsed) !== stableSerialize(effective.contract)) {
            return invalidEffectiveContract("Effective Contract source IR is not a normalized Artifact Contract.", "$.contract");
        }
...
function cloneInputValue(value, path, violations, stack = new WeakSet()) {
...
function validateStringConstraints(value, constraints, path, violations, enumCode = "INPUT_ENUM") {
    ...
    if (constraints?.minLength !== undefined && length < constraints.minLength) {
        addViolation(violations, "INPUT_MIN_LENGTH", path, `Value must contain at least ${constraints.minLength} characters.`);
    }
    if (constraints?.maxLength !== undefined && length > constraints.maxLength) {
        addViolation(violations, "INPUT_MAX_LENGTH", path, `Value must contain at most ${constraints.maxLength} characters.`);
    }
    if (constraints?.pattern !== undefined) {
        ...
        if (!matches)
            addViolation(violations, "INPUT_PATTERN", path, "Value does not match the contract pattern.");
    }
}
function validateIssueReferenceValue(value, path, violations) {
...
function validatePropertyScalar(shape, value, constraints, path, violations) {
    if (shape === "boolean") {
        if (typeof value !== "boolean")
            addViolation(violations, "INPUT_TYPE", path, "Value must be a boolean.");
        return typeof value === "boolean" ? value : undefined;
    }
    if (shape === "issue_reference")
        return validateIssueReferenceValue(value, path, violations);
    if (typeof value !== "string") {
...
function validatePropertyValue(declaration, rawValue, path, violations, output = false) {
    const local = [];
    ...
    if (declaration.cardinality.max === "many") {
        if (!Array.isArray(rawValue)) {
            ...
            rawValue.forEach((entry, index) => {
                ...
                if (value !== undefined || declaration.shape === "boolean")
                    values.push(value);
                if (declaration.shape === "issue_reference" && value !== undefined) {
                    const identity = issueReferenceKey(value);
                    if (identities.has(identity))
                        addViolation(local, output ? "OUTPUT_INVALID" : "INPUT_DUPLICATE", itemPath, "Issue references must be unique.");
...
function validateFieldValue(declaration, rawValue, path, violations, output = false) {
    const local = [];
    ...
    if (declaration.primitive === "text") {
        if (typeof rawValue !== "string")
            addViolation(local, code, path, "Field value must be a string.");
        else
            validateStringConstraints(rawValue, constraints, path, local, "INPUT_OPTION");
        violations.push(...local);
    ...
    rawValue.forEach((entry, index) => {
        ...
        if (!allowed.includes(entry))
            addViolation(local, output ? "OUTPUT_INVALID" : "INPUT_OPTION", itemPath, "Item is not declared by the contract.");
    ...
    if (declaration.primitive === "checklist") {
        for (const item of items) {
            if (item.required && !values.includes(item.id)) {
                addViolation(local, output ? "OUTPUT_INVALID" : "INPUT_CHECKLIST_REQUIRED", path, `Required checklist item "${item.id}" is missing.`);
            }
        }
    }
    violations.push(...local);
...
function declarationValue(declaration, rawValue, path, violations, output = false) {
    if (declaration.presence === "unused") {
        addViolation(violations, output ? "OUTPUT_INVALID" : "INPUT_AUTHORITY", path, "Unused values cannot be materialized.");
        return { valid: false };
    }
    if ("primitive" in declaration)
        return validateFieldValue(declaration, rawValue, path, violations, output);
    return validatePropertyValue(declaration, rawValue, path, violations, output);
}
function readReference(values, reference) {
    const source = values[reference.name];
    if (source === undefined)
        return undefined;
    if (reference.member === undefined)
        return source;
    if (reference.member === "number" && isRecord(source) && typeof source.number === "number")
        return source.number;
    return undefined;
}
function derivationString(value) {
    if (typeof value === "string")
        return value;
    if (typeof value === "boolean")
        return value ? "true" : "false";
...
function evaluateDerivation(derivation, values, violations) {
    if (derivation.operation.op === "copy") {
        const reference = derivation.dependencies[0];
        if (reference === undefined) {
            addViolation(violations, "DERIVATION_INVALID", `$.${derivation.target}`, "Copy derivation metadata is incomplete.");
    ...
    if (derivation.operation.op === "slug") {
        const reference = derivation.dependencies[0];
        if (reference === undefined) {
            addViolation(violations, "DERIVATION_INVALID", `$.${derivation.target}`, "Slug derivation metadata is incomplete.");
            return { resolved: false };
        }
        const source = readReference(values, reference);
    ...
    if (parts === undefined) {
        addViolation(violations, "DERIVATION_INVALID", `$.${derivation.target}`, "Format derivation metadata is incomplete.");
        return { resolved: false };
    }
    let result = "";
...
function materializeResult(effectiveInput, input) {
    const effectiveResult = validateEffectiveContract(effectiveInput);
    if (effectiveResult.effective === undefined)
        return { valid: false, violations: effectiveResult.violations };
    ...
    for (const key of Object.keys(input).sort(compareStrings)) {
        ...
        if (declaration === undefined) {
            addViolation(violations, "INPUT_UNKNOWN_FIELD", `$.${key}`, `Value "${key}" is not declared by the Effective Contract.`);
        }
        else if (declaration.presence === "unused" || authorityOf(declaration) !== "supplied") {
            addViolation(violations, "INPUT_AUTHORITY", `$.${key}`, `Caller cannot supply ${authorityOf(declaration) ?? "unused"} value "${key}".`);
        }
    }
    const values = {};
    const fields = {};
    const materialized = {};
    const assign = (name, declaration, value) => {
    ...
    for (const [name, declaration] of entries) {
        ...
        if (authority.kind === "fixed") {
            ...
            if (result.valid)
                assign(name, declaration, result.value);
            continue;
        }
        if (authority.kind !== "supplied")
            continue;
    ...
    for (const name of effective.evaluationOrder) {
        ...
        if (!evaluated.resolved) {
            if (declaration.presence === "required") {
                addViolation(violations, "DERIVATION_UNRESOLVED", `$.${name}`, `Required derivation "${name}" cannot be resolved.`);
            }
            continue;
        }
...
export function materializeSemanticArtifact(effectiveContract, input) {
    const result = materializeResult(effectiveContract, input);
    if (!result.valid || result.artifact === undefined)
        throw new SemanticArtifactMaterializationError(result.violations);
    return result.artifact;
}
/** Non-throwing companion for callers that need structured preflight diagnostics. */
export function tryMaterializeSemanticArtifact(effectiveContract, input) {
