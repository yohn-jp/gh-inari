 ...
 */
import { normalizeIssueReference } from "./issue-reference.js";
export const ARTIFACT_CONTRACT_VERSION = "1";
export const ARTIFACT_CONTRACT_KINDS = ["issue", "branch", "pull_request"];
const PRESENCE_VALUES = ["required", "optional", "unused"];
...
const VALUE_SHAPE_MEMBERS = {
    issue_reference: ["number"],
};
export const FIELD_PRIMITIVES = ["text", "choice", "checklist", "attachment"];
...
function checkUnknownKeys(record, allowedKeys, path, violations) {
    ...
    for (const key of Object.keys(record)) {
        if (!allowed.has(key)) {
            addViolation(violations, "ARTIFACT_CONTRACT_UNKNOWN_PROPERTY", `${path}.${key}`, `Property "${key}" is not supported.`);
        }
    }
}
function requiredString(record, key, path, violations) {
    ...
    if (typeof value !== "string" || value.trim().length === 0) {
        addViolation(violations, "ARTIFACT_CONTRACT_INVALID_VALUE", `${path}.${key}`, `Property "${key}" must be a non-empty string.`);
        return undefined;
    }
    return value;
}
function optionalNonNegativeInteger(record, key, path, violations) {
    ...
    if (typeof value !== "number" || !Number.isSafeInteger(value) || value < 0) {
        addViolation(violations, "ARTIFACT_CONTRACT_INVALID_CONSTRAINT", `${path}.${key}`, `Property "${key}" must be a non-negative safe integer.`);
        return undefined;
    }
    return value;
}
...
function validateIdentifier(value, path, violations) {
    if (!IDENTIFIER_PATTERN.test(value)) {
        addViolation(violations, "ARTIFACT_CONTRACT_INVALID_IDENTIFIER", path, "Identifiers must be non-empty and contain only letters, numbers, hyphens, or underscores.");
    }
}
...
const KINDS_WITH_FIELDS = new Set(["issue", "pull_request"]);
function cardinalityFor(multiplicity, presence) {
    if (presence === "unused")
        return { min: 0, max: 0 };
    return { min: presence === "required" ? 1 : 0, max: multiplicity === "many" ? "many" : 1 };
}
function validatePresence(value, path, violations) {
    if (typeof value !== "string" || !PRESENCE_VALUES.includes(value)) {
        addViolation(violations, "ARTIFACT_CONTRACT_INVALID_VALUE", path, `Presence "${String(value)}" is not supported; expected required, optional, or unused.`);
        return undefined;
    }
    return value;
}
function validatePropertyConstraints(value, path, shape, multiplicity, violations) {
    ...
    if (minLength !== undefined && maxLength !== undefined && minLength > maxLength) {
        addViolation(violations, "ARTIFACT_CONTRACT_INVALID_CONSTRAINT", path, "minLength cannot be greater than maxLength.");
    }
...
function validateOptionalPattern(record, path, violations) {
    ...
    try {
        ...
        addViolation(violations, "ARTIFACT_CONTRACT_INVALID_CONSTRAINT", `${path}.pattern`, "pattern must be a valid regular expression.");
    }
    return pattern;
}
function validateClosedValueSet(record, path, violations) {
    ...
    if (!Array.isArray(rawValues) ||
        ...
        addViolation(violations, "ARTIFACT_CONTRACT_INVALID_CONSTRAINT", `${path}.values`, "values must be a non-empty array of non-empty strings.");
        return undefined;
    }
...
function validateFixedScalar(value, path, shape, constraints, violations) {
    if (shape === "boolean") {
    ...
    if ((shape === "classification" || shape === "label") && constraints?.values !== undefined) {
        if (!constraints.values.includes(value)) {
            addViolation(violations, "ARTIFACT_CONTRACT_INVALID_FIXED_VALUE", path, "Fixed value must be one of the declared values.");
        }
    }
    if (shape === "text" && constraints?.pattern !== undefined) {
        try {
            if (!new RegExp(constraints.pattern, "u").test(value)) {
                addViolation(violations, "ARTIFACT_CONTRACT_INVALID_FIXED_VALUE", path, "Fixed value does not satisfy pattern.");
            }
        }
        catch {
...
function validateFixedValue(value, path, shape, multiplicity, constraints, violations) {
    if (multiplicity !== "many")
        return validateFixedScalar(value, path, shape, constraints, violations);
...
function validateDerivation(value, path, violations) {
    ...
    if (op === "copy" || op === "slug") {
        ...
        if (typeof from !== "string" || from.length === 0) {
            addViolation(violations, "ARTIFACT_CONTRACT_INVALID_DERIVATION", `${path}.from`, `A ${op} derivation requires a non-empty from reference.`);
            return undefined;
        }
        const reference = parseReference(from);
        if (reference === undefined) {
            addViolation(violations, "ARTIFACT_CONTRACT_INVALID_DERIVATION", `${path}.from`, `Reference "${from}" is not a valid value/member path.`);
            return undefined;
        }
 ...
 */
function validateAuthority(value, path, validateFixed, violations) {
    if (!isRecord(value)) {
    ...
    if (kind === "derived") {
        ...
        if (!hasOwn(value, "derive")) {
            addViolation(violations, "ARTIFACT_CONTRACT_MISSING_PROPERTY", `${path}.derive`, 'Derived authority requires "derive".');
            return undefined;
        }
    ...
    if (kind === "fixed") {
        ...
        if (!hasOwn(value, "value")) {
            addViolation(violations, "ARTIFACT_CONTRACT_MISSING_FIXED_VALUE", `${path}.value`, 'Fixed authority requires "value".');
            return undefined;
        }
    ...
    addViolation(violations, "ARTIFACT_CONTRACT_UNKNOWN_AUTHORITY", `${path}.kind`, `Authority kind "${String(kind)}" is not supported.`);
    return undefined;
}
function validatePropertyDeclaration(value, path, descriptor, violations) {
    if (!isRecord(value)) {
    ...
    if (!hasOwn(value, "presence")) {
        addViolation(violations, "ARTIFACT_CONTRACT_MISSING_PROPERTY", `${path}.presence`, 'Property "presence" is required.');
        return undefined;
    }
    ...
    const constraints = hasOwn(value, "constraints")
        ? validatePropertyConstraints(value.constraints, `${path}.constraints`, descriptor.shape, descriptor.multiplicity, violations)
        : descriptor.shape === "classification"
            ? (addViolation(violations, "ARTIFACT_CONTRACT_MISSING_PROPERTY", `${path}.constraints`, "A classification property requires a closed values set."),
                undefined)
            : undefined;
    if (!hasOwn(value, "authority")) {
        addViolation(violations, "ARTIFACT_CONTRACT_MISSING_PROPERTY", `${path}.authority`, 'Property "authority" is required.');
        return undefined;
    }
    const authority = validateAuthority(value.authority, `${path}.authority`, (fixedValue, fixedPath) => validateFixedValue(fixedValue, fixedPath, descriptor.shape, descriptor.multiplicity, constraints, violations), violations);
...
function validateChecklistItems(value, path, violations) {
    ...
    value.forEach((entry, index) => {
        ...
        if (typeof required !== "boolean") {
            addViolation(violations, "ARTIFACT_CONTRACT_INVALID_VALUE", `${itemPath}.required`, "Checklist item required must be a boolean.");
        }
        if (id !== undefined) {
            validateIdentifier(id, `${itemPath}.id`, violations);
            if (ids.has(id))
                addViolation(violations, "ARTIFACT_CONTRACT_INVALID_VALUE", `${itemPath}.id`, `Duplicate checklist item "${id}".`);
            ids.add(id);
        }
...
function validateFieldConstraints(value, path, primitive, violations) {
    if (!isRecord(value)) {
        addViolation(violations, "ARTIFACT_CONTRACT_INVALID_CONSTRAINT", path, "Constraints must be an object.");
        return undefined;
    }
    const allowedKeys = primitive === "text"
    ...
    if (primitive === "choice" && values === undefined) {
        addViolation(violations, "ARTIFACT_CONTRACT_MISSING_PROPERTY", `${path}.values`, "A choice field requires a closed values set.");
    }
    if (primitive === "checklist" && !hasOwn(value, "items")) {
        addViolation(violations, "ARTIFACT_CONTRACT_MISSING_PROPERTY", `${path}.items`, "A checklist field requires items.");
    }
    if (minLength !== undefined && maxLength !== undefined && minLength > maxLength) {
        addViolation(violations, "ARTIFACT_CONTRACT_INVALID_CONSTRAINT", path, "minLength cannot be greater than maxLength.");
    }
...
function validateFieldDeclaration(value, path, fieldIds, reservedNames, violations) {
    if (!isRecord(value)) {
    ...
    if (id !== undefined) {
        ...
        if (reservedNames.has(id)) {
            addViolation(violations, "ARTIFACT_CONTRACT_INVALID_VALUE", `${path}.id`, `Field id "${id}" collides with a Core-owned property name of this artifact kind.`);
        }
        fieldIds.add(id);
    }
    const primitiveValue = requiredString(value, "primitive", path, violations);
    if (primitiveValue !== undefined && !FIELD_PRIMITIVES.includes(primitiveValue)) {
        addViolation(violations, "ARTIFACT_CONTRACT_UNSUPPORTED_FIELD_PRIMITIVE", `${path}.primitive`, `Field primitive "${primitiveValue}" is not supported.`);
    }
    const primitive = primitiveValue !== undefined && FIELD_PRIMITIVES.includes(primitiveValue)
        ? primitiveValue
        : undefined;
    if (!hasOwn(value, "presence")) {
        addViolation(violations, "ARTIFACT_CONTRACT_MISSING_PROPERTY", `${path}.presence`, 'Property "presence" is required.');
    ...
    const presence = validatePresence(value.presence, `${path}.presence`, violations);
    if (id === undefined || primitive === undefined || presence === undefined)
        return undefined;
    ...
    const constraints = hasOwn(value, "constraints")
        ...
        : primitive === "choice" || primitive === "checklist"
            ? (addViolation(violations, "ARTIFACT_CONTRACT_MISSING_PROPERTY", `${path}.constraints`, `A ${primitive} field requires constraints.`),
                undefined)
            : undefined;
    if (!hasOwn(value, "authority")) {
        addViolation(violations, "ARTIFACT_CONTRACT_MISSING_PROPERTY", `${path}.authority`, 'Property "authority" is required.');
    ...
    const authority = validateAuthority(value.authority, `${path}.authority`, (fixedValue, fixedPath) => {
        if (primitive !== "text") {
            addViolation(violations, "ARTIFACT_CONTRACT_INVALID_FIXED_VALUE", fixedPath, `Fixed authority is only supported for text fields, not "${primitive}".`);
            return undefined;
        }
    ...
    const visit = (name) => {
        ...
        if (current === "visiting") {
            ...
            if (!reportedCycles.has(key)) {
                ...
                addViolation(violations, "ARTIFACT_CONTRACT_DERIVATION_CYCLE", `$.${name}`, `Derivation cycle detected: ${cycle.join(" -> ")}.`);
            }
            return;
        }
        if (!derived.has(name)) {
        ...
        for (const reference of dependenciesByName.get(name) ?? []) {
            if (derived.has(reference.name) || dependenciesByName.has(reference.name))
                visit(reference.name);
        }
        stack.pop();
        state.set(name, "done");
    };
    for (const name of dependenciesByName.keys())
        visit(name);
}
/**
 ...
 */
function compileArtifactContract(input) {
    const violations = [];
    if (!isRecord(input)) {
        return {
            violations: [
    ...
    if (version !== undefined && version !== ARTIFACT_CONTRACT_VERSION) {
        addViolation(violations, "ARTIFACT_CONTRACT_UNSUPPORTED_VERSION", "$.version", `Only Artifact Contract version ${ARTIFACT_CONTRACT_VERSION} is supported.`);
    }
    const kindValue = requiredString(input, "kind", "$", violations);
    const kind = kindValue !== undefined && ARTIFACT_CONTRACT_KINDS.includes(kindValue)
        ? kindValue
        : undefined;
    if (kindValue !== undefined && kind === undefined) {
        addViolation(violations, "ARTIFACT_CONTRACT_UNSUPPORTED_KIND", "$.kind", `Artifact kind "${kindValue}" is not supported.`);
    }
    const id = requiredString(input, "id", "$", violations);
    if (id !== undefined)
        validateIdentifier(id, "$.id", violations);
    checkUnknownKeys(input, kind === undefined || KINDS_WITH_FIELDS.has(kind)
        ? ["version", "kind", "id", "properties", "fields"]
        : ["version", "kind", "id", "properties"], "$", violations);
    if (kind === undefined)
        return { violations };
    const registry = PROPERTY_REGISTRY[kind];
    if (!hasOwn(input, "properties")) {
        addViolation(violations, "ARTIFACT_CONTRACT_MISSING_PROPERTY", "$.properties", 'Property "properties" is required.');
        return { violations };
    }
    const propertiesInput = input.properties;
    if (!isRecord(propertiesInput)) {
        addViolation(violations, "ARTIFACT_CONTRACT_INVALID_VALUE", "$.properties", 'Property "properties" must be an object.');
        return { violations };
    }
    ...
    for (const key of Object.keys(propertiesInput)) {
        ...
        if (descriptor === undefined) {
            addViolation(violations, "ARTIFACT_CONTRACT_UNKNOWN_SEMANTIC_PROPERTY", path, `"${key}" is not a Core-owned semantic property of artifact kind "${kind}".`);
            continue;
        }
...
export function isArtifactContract(input) {
...
export function parseArtifactContract(input) {
    const { violations, contract } = compileArtifactContract(input);
    if (contract === undefined)
        throw new ArtifactContractValidationError(violations);
    return contract;
}
function canonicalizeAuthority(authority) {
    if (authority.kind === "supplied" || authority.kind === "platform")
        return { kind: authority.kind };
    if (authority.kind === "derived")
        return { kind: "derived", derive: { ...authority.derive } };
    return { kind: "fixed", value: authority.value };
}
function canonicalizeProperty(declaration) {
    if (declaration.presence === "unused")
        return { presence: "unused" };
    return {
...
function canonicalizeField(declaration) {
    if (declaration.presence === "unused")
 ...
 */
export function serializeArtifactContract(contract) {
    return JSON.stringify(canonicalizeContract(contract));
}
export function deserializeArtifactContract(serialized) {
    let parsed;
    try {
