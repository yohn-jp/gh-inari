...
const INARI_CHANGE_EXECUTOR_BRANCH = "main";
...
function parseArtifacts(value, expectedName, expectedRepositoryId) {
    const payload = record(value);
    ...
    return payload.artifacts
        ...
        const repositoryId = workflowRun.repository_id === undefined ? undefined : positiveInteger(workflowRun.repository_id);
...
export class GitHubActionsChangeRemoteExecutor {
    #api;
    #cwd;
    #requester;
    #maxPollAttempts;
    ...
    async readCanonicalProjection(request) {
    ...
    async dispatchAndCollect(request) {
        ...
        if (context.repositoryId === undefined) {
            throw remoteError("CHANGE_REMOTE_EXECUTOR_UNAVAILABLE", `change.${request.operation}`, "repository-identity-unavailable");
        }
    ...
    async waitForResult(operation, baseline, artifactName, repositoryId) {
        ...
        for (let attempt = 0; attempt < this.#maxPollAttempts; attempt += 1) {
            ...
            const run = artifact === undefined ? undefined : candidates.find((candidate) => candidate.id === artifact.workflowRunId);
    ...
    async readArtifacts(operation, name, repositoryId) {
        let value;
        try {
...
class GitHubRepositoryReadTransport {
    ...
    constructor(api, context, requestRepositoryApi) {
        this.#api = api;
...
function boundedOption(value, minimum, maximum) {
    if (!Number.isSafeInteger(value) || value < minimum || value > maximum) {
        throw new RangeError("GitHub Actions polling options are outside their bounded range.");
    }
    return value;
}
export function createGitHubActionsChangeRemoteExecutor(options) {
    return new GitHubActionsChangeRemoteExecutor(options);
}
//# sourceMappingURL=change-actions-remote-executor.js.map