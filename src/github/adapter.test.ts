 ...
 */
class ArtifactFixtureTransport implements FixtureCommandTransport {
...
test("reads an Actions artifact through bounded binary stdout", async () => {
  const transport = new ArtifactFixtureTransport("regular");
...
test("clamps an Actions artifact download to the remaining Change budget", async () => {
  ...
  // The native transport receives the shared deadline through the adapter's
  // bounded promise; command-fixture timeout metadata is not part of the seam.
  assert.equal(transport.calls.at(-1)?.timeoutMs, 10_000);
});

test("fails closed for missing and oversized binary artifact responses", async () => {
  for (const mode of ["missing", "oversized"] as const) {
    const transport = new ArtifactFixtureTransport(mode);
    const adapter = new GitHubAdapter({ repository: "acme/inari", transport: nativeTestTransport(transport) });
