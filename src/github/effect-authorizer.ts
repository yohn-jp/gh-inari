/**
 * The Inari App Principal Effect Authorizer boundary.
 *
 * This module owns the App Principal identity, scope, permission, and
 * trusted-execution checks around one already-planned provider effect. It
 * deliberately does not derive branches, validate lifecycle transitions, or
 ...
 *
 * `issuer` remains in the request/result contracts because it is an existing
 * wire and provenance compatibility value. It does not designate repository
 * Authority; the canonical component role is Effect Authorizer.
 */
...
export const EFFECT_AUTHORIZER_CONTRACT_VERSION = 1 as const;
export type EffectAuthorizerContractVersion = typeof EFFECT_AUTHORIZER_CONTRACT_VERSION;

/** @deprecated Use `EFFECT_AUTHORIZER_CONTRACT_VERSION`. */
export const ISSUER_AUTHORITY_CONTRACT_VERSION = EFFECT_AUTHORIZER_CONTRACT_VERSION;
/** @deprecated Use `EffectAuthorizerContractVersion`. */
export type IssuerAuthorityContractVersion = EffectAuthorizerContractVersion;
...
export const APP_PERMISSION_NAMES = ISSUER_PERMISSION_NAMES;
...
export type AppPermissionName = IssuerPermissionName;
export type AppPermissionAccess = IssuerPermissionAccess;
export type AppPermissionSet = IssuerPermissionSet;
export type AppPrincipalPermissionSet = AppPermissionSet;
 ...
 * review, or merge permission is part of the App Principal effect ceiling.
 */
...
export const APP_PRINCIPAL_MAXIMUM_PERMISSIONS = INARI_ISSUER_MAXIMUM_PERMISSIONS;
...
/** @deprecated Use `RepositoryIdentity`. */
export type IssuerRepositoryIdentity = RepositoryIdentity;

export interface AppPrincipalIdentity {
...
export type AppPrincipal = AppPrincipalIdentity;
export type InariAppPrincipalIdentity = AppPrincipalIdentity;
/** @deprecated Use `AppPrincipalIdentity`. */
export type InariIssuerAppIdentity = AppPrincipalIdentity;

export interface AppInstallationIdentity {
  readonly appId: string;
  readonly installationId: string;
  readonly repositoryHost: string;
}

/** @deprecated Use `AppInstallationIdentity`. */
export type IssuerInstallationIdentity = AppInstallationIdentity;
 ...
 */
export interface AppInstallationScope {
  readonly app: AppPrincipalIdentity;
  readonly installation: AppInstallationIdentity;
...
/** @deprecated Use `AppInstallationScope`. */
export type IssuerInstallationScope = AppInstallationScope;
...
export interface GitHubActionsTrustedExecutionContext {
  ...
  /** No caller-controlled repository code is executed for App Principal effects. */
...
export type EffectAuthorizerDiagnosticCode = IssuerDiagnosticCode;
export type EffectAuthorizerDiagnostic = IssuerDiagnostic;
export type EffectAuthorizerDiagnosticReport = IssuerDiagnosticReport;
export type EffectAuthorizerValidationResult<T> = IssuerValidationResult<T>;
export type EffectAuthorizerMutationRequest = IssuerMutationRequest;
export type ValidatedEffectAuthorizerMutationRequest = ValidatedIssuerMutationRequest;
export type EffectAuthorizerCredentialRequest = IssuerCredentialRequest;
export type AppScopedMutationCapability = IssuerScopedMutationCapability;
export type EffectAuthorizerCredentialBroker = TrustedInstallationCredentialBroker;
export type EffectAuthorizerInstallationScope = AppInstallationScope;
export type EffectAuthorizerTrustedExecutionContext = TrustedExecutionContext;
export type EffectAuthorizerMutationReceipt = IssuerMutationReceipt;
export type EffectAuthorizerMutationResult = IssuerMutationResult;
export type AppInstallationScopeValidationOptions = IssuerInstallationScopeValidationOptions;

export interface EffectAuthorizerOptions {
...
export type InariEffectAuthorizerOptions = EffectAuthorizerOptions;

/** @deprecated Use `EffectAuthorizerOptions`. */
export type InariIssuerAppAuthorityOptions = EffectAuthorizerOptions;
...
function validateRepositoryIdentityInput(
  input: unknown,
  path: string,
...
export function validateIssuerRepositoryIdentity(
  ...
  return validateRepositoryIdentityInput(input, path);
}

export function createInariAppPrincipalIdentity(appId: string): AppPrincipalIdentity {
  const result = validateInariAppPrincipalIdentity({
...
/** @deprecated Use `createInariAppPrincipalIdentity`. */
export const createInariIssuerAppIdentity = createInariAppPrincipalIdentity;
/** @deprecated Use `createInariAppPrincipalIdentity`. */
export const createInariIssuerAppPrincipal = createInariAppPrincipalIdentity;
export const createInariAppPrincipal = createInariAppPrincipalIdentity;
export const createAppPrincipalIdentity = createInariAppPrincipalIdentity;
export const createAppPrincipal = createInariAppPrincipalIdentity;

export function validateInariAppPrincipalIdentity(
  input: unknown,
  path = "$.app",
): IssuerValidationResult<AppPrincipalIdentity> {
  const diagnostics: IssuerDiagnostic[] = [];
  if (!isRecord(input))
...
/** @deprecated Use `validateInariAppPrincipalIdentity`. */
export const validateInariIssuerAppIdentity = validateInariAppPrincipalIdentity;
export const validateAppPrincipalIdentity = validateInariAppPrincipalIdentity;
export const validateInariAppPrincipal = validateInariAppPrincipalIdentity;
export const validateAppPrincipal = validateInariAppPrincipalIdentity;
...
export function validateIssuerMutationRequest(input: unknown): IssuerValidationResult<ValidatedIssuerMutationRequest> {
  ...
  const targetResult = requireProperty(input, "target", "$", diagnostics)
    ? validateRepositoryIdentityInput(input.target, "$.target")
    : report<IssuerRepositoryIdentity>([]);
...
export function validateIssuerInstallationScope(
  ...
  const repositoryResult = requireProperty(input, "repository", "$.scope", diagnostics)
    ? validateRepositoryIdentityInput(input.repository, "$.scope.repository")
    : report<IssuerRepositoryIdentity>([]);
  diagnostics.push(...repositoryResult.diagnostics);
...
export const validateAppInstallationScope = validateIssuerInstallationScope;
export const validateEffectAuthorizerMutationRequest = validateIssuerMutationRequest;
export const validateRepositoryIdentity = validateIssuerRepositoryIdentity;
...
function assertCredentialRequest(request: IssuerCredentialRequest): IssuerCredentialRequest {
  ...
  const target = throwValidation(validateRepositoryIdentityInput(request.target, "$.credential.target"));
 ...
 */
export class InariEffectAuthorizer {
  readonly appPrincipal: AppPrincipalIdentity;
  /** @deprecated Use `appPrincipal`. */
  readonly identity: AppPrincipalIdentity;
  #broker: TrustedInstallationCredentialBroker;
  #now: () => Date;

  constructor(options: EffectAuthorizerOptions) {
    ...
    this.appPrincipal = createInariAppPrincipalIdentity(options.appId);
    this.identity = this.appPrincipal;
...
/**
 * Canonical role name for the component that admits already-planned effects.
 * This method is an explicit vocabulary alias; it does not create a second
 * policy or mutation path.
 */
export async function authorizeEffects(
  authorizer: Pick<InariEffectAuthorizer, "applyEffects">,
  input: unknown,
): Promise<IssuerMutationResult> {
  return authorizer.applyEffects(input);
}

export class EffectAuthorizerError extends Error {
  readonly code: IssuerDiagnosticCode;
  readonly path: string;
  readonly diagnostics: readonly IssuerDiagnostic[];
  ...
  constructor(
    ...
    super(first.message);
    this.name = "EffectAuthorizerError";
...
/** @deprecated Use `InariEffectAuthorizer`. */
export const InariIssuerAppAuthority = InariEffectAuthorizer;
/** @deprecated Use `InariEffectAuthorizer`. */
export type InariIssuerAppAuthority = InariEffectAuthorizer;
/** @deprecated Use `InariEffectAuthorizer`. */
export const IssuerAppAuthority = InariEffectAuthorizer;
/** @deprecated Use `InariEffectAuthorizer`. */
export type IssuerAppAuthority = InariEffectAuthorizer;
export const EffectAuthorizer = InariEffectAuthorizer;
export type EffectAuthorizer = InariEffectAuthorizer;
/** @deprecated Use `EffectAuthorizerError`. */
export const IssuerAuthorityError = EffectAuthorizerError;
/** @deprecated Use `EffectAuthorizerError`. */
export type IssuerAuthorityError = EffectAuthorizerError;
