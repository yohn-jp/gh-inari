...
import {
  ...
  schemaMinItems,
  type EffectiveFieldConstraints,
...
import { projectToJsonSchema, type JsonSchema, type JsonSchemaDocument } from "./contract/schema.js";
...
type RequiredPrSyncPropertyName = {
  [Name in PrSyncPropertyName]: (typeof PR_SYNC_INPUT_CONTRACT.properties)[Name]["required"] extends true
    ? Name
    : never;
}[PrSyncPropertyName];
type OptionalPrSyncPropertyName = Exclude<PrSyncPropertyName, RequiredPrSyncPropertyName>;
type PrSyncValue<Definition extends PrSyncPropertyDefinition> = Definition["valueKind"] extends "semantic-fields"
  ? Readonly<Record<string, unknown>>
  : Definition["valueKind"] extends "non-empty-string"
    ? string
    : boolean;
...
const requiredPropertyNames = Object.entries(PR_SYNC_INPUT_CONTRACT.properties)
  ...
  .map(([name]) => name) as readonly RequiredPrSyncPropertyName[];

const optionalPropertyNames = Object.entries(PR_SYNC_INPUT_CONTRACT.properties)
  .filter(([, definition]) => !definition.required)
  .map(([name]) => name) as readonly OptionalPrSyncPropertyName[];

/** Return the bounded top-level shape used by help and machine consumers. */
export function projectPullRequestSyncInput(contractInput: unknown): PullRequestSyncInputProjection {
...
export function minimalPullRequestSyncInput(contractInput: unknown): PullRequestSyncInput {
  assertCanonicalContract(contractInput);
  if (contractInput.artifactKind !== "pull_request") {
    throw new ArtifactInputError("INPUT_DOCUMENT_INVALID", "A pull request contract is required.");
  }
  ...
  for (const field of contractInput.sections.flatMap((section) => section.fields)) {
    ...
    if (!constraints.required && !constraints.hasDefault) continue;
    if (constraints.hasDefault) continue;
    fields[field.id] = minimalFieldValue(field, constraints);
  }
  const validation = validateSemanticInput(contractInput, fields);
  if (!validation.valid) {
    throw new Error(
      `Could not generate a valid pull-request sync example: ${validation.violations.map((violation) => violation.path).join(", ")}`,
    );
  }
  return {
...
export function parsePullRequestSyncInput(input: unknown): ArtifactInputDocument {
  if (!isRecord(input)) {
    throw prSyncInputError("$", "Pull-request sync input must be a JSON object.", "invalid", input);
  }
  const unknown = Object.keys(input).find(
    (key) => !Object.prototype.hasOwnProperty.call(PR_SYNC_INPUT_CONTRACT.properties, key),
  );
  if (unknown !== undefined) {
    throw prSyncInputError(
      `$.${unknown}`,
      `Unknown pull-request sync input property "${unknown}".`,
      "invalid",
      input[unknown],
    );
  }
...
export function assertPullRequestSyncInputComplete(input: ArtifactInputDocument): ArtifactInputDocument {
  const candidate: Record<string, unknown> = { fields: input.fields, ...input.metadata };
  for (const name of requiredPropertyNames) {
...
export function renderPullRequestSyncInputHelp(): string {
  ...
  const optional = optionalPropertyNames
    ...
    .join(", ");
  return (
    `The --from JSON contract is one object with required top-level properties: ${required}. ` +
...
function minimalFieldValue(field: CanonicalField, constraints: EffectiveFieldConstraints): unknown {
  if (field.type === "enum") return field.options[0]?.value ?? "example";
  if (field.type === "array") {
...
function minimalArrayValues(allowedValues: readonly string[], minimum: number, maxItems: number | undefined): string[] {
  const values: string[] = [];
  let index = 0;
...
function minimalStringValue(constraints: EffectiveFieldConstraints): string {
