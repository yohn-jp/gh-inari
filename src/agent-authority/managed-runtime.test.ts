...
import {
  ...
  type ManagedRuntimeSessionOptions,
} from "./managed-runtime.js";
import { sendDirectAppRequest } from "./direct-app-client.js";
import { SessionBootstrapError, SessionCertificateIssuanceError } from "./session-issuance.js";
...
function options(
  ...
  overrides: Partial<ManagedRuntimeSessionOptions> = {},
): ManagedRuntimeSessionOptions {
  return {
...
test("managed-runtime seam returns only public Session/certificate material", () => {
  ...
  const managed = createManagedRuntimeSession(
    options(runtimeKey, {
...
test("the composition signs the canonical Session request and feeds the direct App client unchanged", async () => {
  const runtimeKey = generateRuntimeAuthorityKeyPair();
  const managed = createManagedRuntimeSession(options(runtimeKey));
...
test("parallel managed Sessions have non-substitutable certificate identities", () => {
  const runtimeKey = generateRuntimeAuthorityKeyPair();
  const first = createManagedRuntimeSession(options(runtimeKey));
  const second = createManagedRuntimeSession(options(runtimeKey));
...
test("provenance cannot change certificate authorization claims", () => {
  const runtimeKey = generateRuntimeAuthorityKeyPair();
  const withLabels = createManagedRuntimeSession(
    options(runtimeKey, { provenance: { runtime: "runtime-a", workspace: "workspace-a" } }),
  );
  const withoutLabels = createManagedRuntimeSession(options(runtimeKey));
...
test("managed-runtime composition preserves existing TTL and capability admission rules", () => {
  ...
  assert.throws(
    () => createManagedRuntimeSession(options(runtimeKey, { runtimeAuthority: limitedAuthority, ttlSeconds: 121 })),
    (error: unknown) =>
...
test("provenance validation is closed-world and bounded", () => {
  const runtimeKey = generateRuntimeAuthorityKeyPair();
  assert.throws(
    () => createManagedRuntimeSession(options(runtimeKey, { provenance: { unsupported: "label" } as never })),
    (error: unknown) =>
      error instanceof ManagedRuntimeSessionError && error.code === "MANAGED_RUNTIME_INVALID_PROVENANCE",
  );
  assert.throws(
    () => createManagedRuntimeSession(options(runtimeKey, { provenance: { runtime: "x".repeat(129) } })),
    (error: unknown) =>
      error instanceof ManagedRuntimeSessionError && error.code === "MANAGED_RUNTIME_INVALID_PROVENANCE",
  );
});
