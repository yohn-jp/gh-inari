...
export type CanonicalJsonValue =
  CanonicalJsonPrimitive | readonly CanonicalJsonValue[] | { readonly [key: string]: CanonicalJsonValue };

/** Defends against pathological input; every real signing structure here nests a handful of levels. */
export const MAX_CANONICAL_JSON_DEPTH = 32 as const;
...
function serialize(value: CanonicalJsonValue, depth: number): string {
  ...
  if (typeof value === "object") return serializeObject(value as Readonly<Record<string, CanonicalJsonValue>>, depth);
  throw new TypeError("Unsupported canonical JSON value.");
}

function serializeObject(value: Readonly<Record<string, CanonicalJsonValue>>, depth: number): string {
  const keys = Object.keys(value).sort(compareUtf16CodeUnits);
  const entries = keys.map((key) => `${JSON.stringify(key)}:${serialize(value[key] as CanonicalJsonValue, depth + 1)}`);
  return `{${entries.join(",")}}`;
}
 ...
 */
export function base64UrlDecodeToBytes(value: string): Buffer {
