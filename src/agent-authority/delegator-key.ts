/**
 * Local Delegator key management.
 *
 * This module owns only the local Ed25519 delegation key. It deliberately
 * does not create, write, or register a Delegator trust record and it
 * has no GitHub/App transport. The public key returned here is suitable for a
 * #367 Delegator record; repository trust remains a separate governed
...
export const DELEGATOR_PRIVATE_KEY_FILE_FORMAT = "PKCS#8-PEM" as const;
/** @deprecated Compatibility spelling for the private-key encoding. */
export const RUNTIME_PRIVATE_KEY_FILE_FORMAT = DELEGATOR_PRIVATE_KEY_FILE_FORMAT;

/** A malformed or unsafe key file must not be allowed to consume unbounded local input. */
export const MAX_DELEGATOR_PRIVATE_KEY_FILE_BYTES = 16 * 1024;
/** @deprecated Compatibility spelling for the private-key input bound. */
export const MAX_RUNTIME_PRIVATE_KEY_FILE_BYTES = MAX_DELEGATOR_PRIVATE_KEY_FILE_BYTES;

export type DelegatorKeyErrorCode =
...
export class DelegatorKeyError extends Error {
  readonly code: DelegatorKeyErrorCode;

  constructor(code: DelegatorKeyErrorCode, message: string) {
    super(message);
    this.name = "DelegatorKeyError";
    this.code = code;
  }
}

export interface DelegatorKeyPair {
  /** Local-only delegation signer. Never include this field in public output. */
  readonly privateKey: KeyObject;
  readonly publicKey: KeyObject;
  /** Public JWK compatible with #367's Delegator `key` property. */
  readonly publicKeyJwk: Ed25519PublicJwk;
}

export interface PersistDelegatorPrivateKeyOptions {
...
function isKeyPair(value: KeyObject | DelegatorKeyPair): value is DelegatorKeyPair {
  return typeof value === "object" && value !== null && "privateKey" in value && "publicKey" in value;
}

function safeKeyError(code: DelegatorKeyErrorCode, message: string): DelegatorKeyError {
  return new DelegatorKeyError(code, message);
}
...
function assertPrivateEd25519Key(key: KeyObject): void {
  if (typeof key !== "object" || key === null || key.type !== "private" || key.asymmetricKeyType !== "ed25519") {
    throw safeKeyError(
      "RUNTIME_AUTHORITY_KEY_INVALID_PRIVATE_KEY",
      "Delegator private key must be an Ed25519 private key.",
    );
  }
}
...
function publicJwkFromKey(key: KeyObject): Ed25519PublicJwk {
  try {
    ...
    if (error instanceof DelegatorKeyError) throw error;
    throw safeKeyError(
      "RUNTIME_AUTHORITY_KEY_INVALID_PRIVATE_KEY",
      "Delegator key did not contain a valid Ed25519 public key.",
    );
  }
}

function pairFromPrivateKey(privateKey: KeyObject): DelegatorKeyPair {
...
/** Generate an Ed25519 Delegator keypair entirely in local process memory. */
export function generateDelegatorKeyPair(): DelegatorKeyPair {
  try {
    const { privateKey } = generateKeyPairSync("ed25519");
    return pairFromPrivateKey(privateKey);
  } catch {
    throw safeKeyError("RUNTIME_AUTHORITY_KEY_INVALID_PRIVATE_KEY", "Unable to generate an Ed25519 Delegator keypair.");
  }
}

/**
 * Import the trusted Delegator's existing PKCS#8 PEM secret without
 * changing the key format used by the file-backed Delegator key mechanism.
 */
export function importDelegatorPrivateKey(pem: string): KeyObject {
  if (
    typeof pem !== "string" ||
    pem.length === 0 ||
    Buffer.byteLength(pem, "utf8") > MAX_DELEGATOR_PRIVATE_KEY_FILE_BYTES
  ) {
    throw safeKeyError("RUNTIME_AUTHORITY_KEY_INVALID_PRIVATE_KEY", "Delegator private key is too large or empty.");
  }
  try {
    ...
    throw safeKeyError(
      "RUNTIME_AUTHORITY_KEY_INVALID_PRIVATE_KEY",
      "Delegator private key is malformed or not Ed25519.",
    );
  }
}

/** Return only the public JWK, whether the input is a public or private KeyObject or a generated pair. */
export function exportDelegatorPublicKey(key: KeyObject | DelegatorKeyPair): Ed25519PublicJwk {
  if (isKeyPair(key)) return assertEd25519PublicJwk(key.publicKeyJwk, "$.publicKey");
  return publicJwkFromKey(key);
}

/** Deterministic public-key JSON for embedding as #367's Delegator `key` value. */
export function canonicalDelegatorPublicKeyJson(key: Ed25519PublicJwk | KeyObject | DelegatorKeyPair): string {
  const publicKey =
    typeof key === "object" && key !== null && "kty" in key
      ? assertEd25519PublicJwk(key, "$.publicKey")
      : exportDelegatorPublicKey(key as KeyObject | DelegatorKeyPair);
  return canonicalJsonString(publicKey as unknown as CanonicalJsonValue);
}

/**
 * Stable, public identity for a Delegator key.
 *
 ...
 * unambiguous without changing the Delegator schema or `kid` value.
 */
export function delegatorPublicKeyFingerprint(key: Ed25519PublicJwk | KeyObject | DelegatorKeyPair): string {
  return `sha256:${createHash("sha256").update(canonicalDelegatorPublicKeyJson(key), "utf8").digest("hex")}`;
}
...
function privateKeyPem(key: KeyObject): string {
  ...
  try {
    ...
    if (error instanceof DelegatorKeyError) throw error;
    throw safeKeyError("RUNTIME_AUTHORITY_KEY_INVALID_PRIVATE_KEY", "Unable to encode the Delegator private key.");
  }
}
...
function assertSecureDirectoryStat(stat: Stats): void {
  ...
  if (
    ...
    throw safeKeyError("RUNTIME_AUTHORITY_KEY_UNSAFE_STORAGE", "Delegator key directory is not private.");
  }
}
...
function assertSafeAncestorDirectoryStat(stat: Stats): void {
  ...
  if (
    ...
    throw safeKeyError(
      "RUNTIME_AUTHORITY_KEY_UNSAFE_STORAGE",
      "Delegator key path contains an unsafe ancestor directory.",
    );
  }
}
...
function noFollowFlag(): number {
  if (typeof fsConstants.O_NOFOLLOW !== "number") {
    throw safeKeyError(
      "RUNTIME_AUTHORITY_KEY_UNSAFE_STORAGE",
      "This platform cannot safely reject Delegator key symlinks.",
    );
  }
...
function requiredDescriptorPath(fd: number, child?: string): string {
  ...
  if (result === undefined) {
    throw safeKeyError(
      "RUNTIME_AUTHORITY_KEY_UNSAFE_STORAGE",
      "This platform cannot safely anchor Delegator key paths to a directory descriptor.",
    );
  }
  return result;
}
  ...
  try {
    for (const component of components) {
      ...
      try {
        ...
        if (
          !createMissing ||
          (error instanceof DelegatorKeyError && error.code !== "RUNTIME_AUTHORITY_KEY_UNSAFE_STORAGE")
        ) {
          throw error;
        }
        try {
          ...
          if ((mkdirError as NodeJS.ErrnoException).code !== "EEXIST") {
            throw safeKeyError(
              "RUNTIME_AUTHORITY_KEY_STORAGE_FAILED",
              "Unable to prepare the Delegator key directory.",
            );
          }
        }
        childFd = openAndValidateAncestorDirectory(childPath, noFollow);
...
function targetBasename(filePath: string): string {
  ...
  if (basename.length === 0 || basename === "." || basename === "..") {
    throw safeKeyError("RUNTIME_AUTHORITY_KEY_INVALID_PATH", "Delegator private key path is required.");
  }
  return basename;
}
...
function assertSecurePrivateKeyStat(stat: Stats): void {
  ...
  if (
    ...
    throw safeKeyError(
      "RUNTIME_AUTHORITY_KEY_UNSAFE_STORAGE",
      "Delegator private key file permissions or ownership are unsafe.",
    );
  }
  if (stat.size > MAX_DELEGATOR_PRIVATE_KEY_FILE_BYTES) {
    throw safeKeyError("RUNTIME_AUTHORITY_KEY_UNSAFE_STORAGE", "Delegator private key file is too large.");
  }
}
...
function existingPrivateKeyStat(filePath: string): Stats | undefined {
  try {
    ...
    throw safeKeyError(
      "RUNTIME_AUTHORITY_KEY_UNSAFE_STORAGE",
      "Delegator private key path cannot be inspected safely.",
    );
  }
}
...
/**
 * Persist only the Delegator private key in a restrictive local secret file.
 * This function never writes a Delegator record or any repository
 * path. Replacement is explicit and uses an atomic same-directory rename.
 */
export function persistDelegatorPrivateKey(
  filePath: string,
  key: KeyObject,
  options: PersistDelegatorPrivateKeyOptions = {},
): string {
  ...
  try {
    ...
    if (existing !== undefined) {
      if (existing.isSymbolicLink() || !existing.isFile() || !isOwner(existing)) {
        throw safeKeyError("RUNTIME_AUTHORITY_KEY_UNSAFE_STORAGE", "Delegator private key path is unsafe.");
      }
      if (!options.replace) {
        throw safeKeyError(
          "RUNTIME_AUTHORITY_KEY_EXISTS",
          "Delegator private key already exists; use explicit replacement.",
        );
      }
    ...
    if (!options.replace) {
      ...
      try {
        ...
        throw safeKeyError("RUNTIME_AUTHORITY_KEY_STORAGE_FAILED", "Unable to persist the Delegator private key.");
      } finally {
        closeQuietly(fd);
    ...
    try {
      ...
      throw safeKeyError("RUNTIME_AUTHORITY_KEY_STORAGE_FAILED", "Unable to replace the Delegator private key.");
    }
    return targetPath;
  } finally {
...
export function generateAndPersistDelegatorKeyPair(
  filePath: string,
  options: PersistDelegatorPrivateKeyOptions = {},
): DelegatorKeyPair {
  const pair = generateDelegatorKeyPair();
  persistDelegatorPrivateKey(filePath, pair.privateKey, options);
  return pair;
}

/**
 * Load a local Delegator private key with descriptor-level no-follow and
 ...
 */
export function loadDelegatorPrivateKey(filePath: string): KeyObject {
  if (typeof filePath !== "string" || filePath.trim().length === 0) {
    throw safeKeyError("RUNTIME_AUTHORITY_KEY_INVALID_PATH", "Delegator private key path is required.");
  }
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      throw safeKeyError("RUNTIME_AUTHORITY_KEY_NOT_FOUND", "Delegator private key file was not found.");
    }
    throw safeKeyError("RUNTIME_AUTHORITY_KEY_UNSAFE_STORAGE", "Delegator private key file cannot be opened safely.");
  }
  ...
  try {
    ...
    const content = Buffer.alloc(MAX_DELEGATOR_PRIVATE_KEY_FILE_BYTES + 1);
    let offset = 0;
    ...
    if (offset > MAX_DELEGATOR_PRIVATE_KEY_FILE_BYTES) {
      throw safeKeyError("RUNTIME_AUTHORITY_KEY_UNSAFE_STORAGE", "Delegator private key file is too large.");
    }
    ...
    try {
      ...
      throw safeKeyError(
        "RUNTIME_AUTHORITY_KEY_INVALID_PRIVATE_KEY",
        "Delegator private key file is malformed or not Ed25519.",
      );
    }
  } finally {
    try {
...
export function loadDelegatorKeyPair(filePath: string): DelegatorKeyPair {
  return pairFromPrivateKey(loadDelegatorPrivateKey(filePath));
}

/** Default local secret-file location used by the CLI when no path is supplied. */
export function defaultDelegatorPrivateKeyPath(): string {
  return path.join(os.homedir(), ".config", "inari", "runtime-authority.pem");
}

/** Compatibility aliases for the pre-Delegator local key API. */
export type RuntimeAuthorityKeyErrorCode = DelegatorKeyErrorCode;
export { DelegatorKeyError as RuntimeAuthorityKeyError };
export type RuntimeAuthorityKeyPair = DelegatorKeyPair;
export type PersistRuntimeAuthorityPrivateKeyOptions = PersistDelegatorPrivateKeyOptions;
export const generateRuntimeAuthorityKeyPair = generateDelegatorKeyPair;
export const importRuntimeAuthorityPrivateKey = importDelegatorPrivateKey;
export const exportRuntimeAuthorityPublicKey = exportDelegatorPublicKey;
export const canonicalRuntimeAuthorityPublicKeyJson = canonicalDelegatorPublicKeyJson;
export const runtimeAuthorityPublicKeyFingerprint = delegatorPublicKeyFingerprint;
export const persistRuntimeAuthorityPrivateKey = persistDelegatorPrivateKey;
export const generateAndPersistRuntimeAuthorityKeyPair = generateAndPersistDelegatorKeyPair;
export const loadRuntimeAuthorityPrivateKey = loadDelegatorPrivateKey;
export const loadRuntimeAuthorityKeyPair = loadDelegatorKeyPair;
export const defaultRuntimeAuthorityPrivateKeyPath = defaultDelegatorPrivateKeyPath;
