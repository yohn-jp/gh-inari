...
const CHANGE_CAPABILITY_KINDS: ReadonlySet<CapabilityKind> = new Set([
  "change.implement",
  "change.ready",
  "change.abort",
]);
...
export const MAX_PATH_POLICY_LENGTH = 160 as const;
...
export type CapabilityClaim =
  ChangeCapabilityClaim | BranchCreateCapabilityClaim | BranchAdvanceCapabilityClaim | PullRequestCreateCapabilityClaim;

export type CapabilityDiagnosticCode =
  | "CAPABILITY_INVALID_ROOT"
  | "CAPABILITY_MISSING_PROPERTY"
  | "CAPABILITY_UNKNOWN_PROPERTY"
  | "CAPABILITY_UNSUPPORTED_KIND"
  | "CAPABILITY_INVALID_CLAIM";
...
function diagnostic(code: CapabilityDiagnosticCode, path: string, message: string): CapabilityDiagnostic {
...
function isSafeIssueNumber(value: unknown): value is number {
...
export function validateCapabilityClaim(input: unknown, path = "$"): CapabilityClaimValidationResult {
    ...
    if (diagnostics.length === 0) {
      diagnostics.push(
        diagnostic(
          "CAPABILITY_UNSUPPORTED_KIND",
          `${path}.kind`,
          "Capability kind is not a recognized semantic capability.",
        ),
      );
    }
  ...
  if (CHANGE_CAPABILITY_KINDS.has(kind)) {
    ...
    if (requireProperty(input, "issue", path, diagnostics) && !isSafeIssueNumber(input.issue)) {
      diagnostics.push(
        diagnostic("CAPABILITY_INVALID_CLAIM", `${path}.issue`, "issue must be a positive integer Issue number."),
      );
    }
    if (diagnostics.length > 0) return { valid: false, diagnostics };
    return {
      valid: true,
      value: Object.freeze({ kind, issue: input.issue as number }) as ChangeCapabilityClaim,
      diagnostics: [],
    };
  }
  ...
  if (kind === "branch.create") {
    ...
    if (requireProperty(input, "max", path, diagnostics) && input.max !== CAPABILITY_CREATE_MAX) {
      diagnostics.push(
        diagnostic("CAPABILITY_INVALID_CLAIM", `${path}.max`, `max must be exactly ${CAPABILITY_CREATE_MAX}.`),
      );
    }
    if (diagnostics.length > 0) return { valid: false, diagnostics };
    return {
      ...
      }) as BranchCreateCapabilityClaim,
      diagnostics: [],
    };
  }
  ...
  if (kind === "branch.advance") {
    ...
    if ("pathPolicy" in input) {
      if (
        typeof input.pathPolicy !== "string" ||
        ...
        diagnostics.push(
          diagnostic(
            "CAPABILITY_INVALID_CLAIM",
            `${path}.pathPolicy`,
            `pathPolicy must be 1-${MAX_PATH_POLICY_LENGTH} characters.`,
          ),
        );
      } else {
    ...
    return {
      ...
      }) as BranchAdvanceCapabilityClaim,
      diagnostics: [],
    };
  }
  ...
  return {
    ...
    value: Object.freeze({
      ...
      max: CAPABILITY_CREATE_MAX,
    }) as PullRequestCreateCapabilityClaim,
    diagnostics: [],
  };
}

/** `EffectiveAuthority` intersection (architecture doc 7.3/11.1): a claim's kind must be within the delegating Runtime's ceiling. */
export function capabilityClaimWithinCeiling(claim: CapabilityClaim, ceiling: readonly CapabilityKind[]): boolean {
  return ceiling.includes(claim.kind);
}

/** The Issue number a change.* claim is scoped to, for task-binding consistency checks. */
export function capabilityClaimIssueNumber(claim: CapabilityClaim): number | undefined {
  return CHANGE_CAPABILITY_KINDS.has(claim.kind) ? (claim as ChangeCapabilityClaim).issue : undefined;
}
