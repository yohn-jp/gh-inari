...
export const SEMANTIC_BRANCH_EXECUTOR_CONTRACT_VERSION = "1" as const;
export type SemanticBranchExecutorContractVersion = typeof SEMANTIC_BRANCH_EXECUTOR_CONTRACT_VERSION;

export const SEMANTIC_BRANCH_EXECUTION_OUTCOMES = Object.freeze(["verified", "failed"] as const);
...
export interface SemanticBranchExecutionEvidence {
  ...
  readonly branch?: Readonly<{ readonly name: string; readonly sha: string }>;
...
export type SemanticBranchExecutorErrorCode =
  | "SEMANTIC_BRANCH_EXECUTION_REQUEST_INVALID"
  | "SEMANTIC_BRANCH_EXECUTION_PLAN_INVALID"
  | "SEMANTIC_BRANCH_EXECUTION_PRECONDITION_FAILED"
  | "SEMANTIC_BRANCH_EXECUTION_REVALIDATION_FAILED"
  | "SEMANTIC_BRANCH_EXECUTION_EFFECT_FAILED"
  | "SEMANTIC_BRANCH_EXECUTION_READ_FAILED"
  | "SEMANTIC_BRANCH_EXECUTION_PROJECTION_VERIFICATION_FAILED";
...
export class SemanticBranchExecutorError extends Error {
  ...
  constructor(
    code: SemanticBranchExecutorErrorCode,
    message: string,
    diagnostics: readonly SemanticBranchExecutionDiagnostic[] = [],
    details?: Readonly<Record<string, unknown>>,
    evidence?: SemanticBranchExecutionEvidence,
  ) {
    super(message);
...
function diagnostic(code: string, path: string, message: string): SemanticBranchExecutionDiagnostic {
...
function effectiveGeneration(effective: { readonly generation: ArtifactContractProvenance }): string {
  return stableSerialize(effective.generation);
}

function planGeneration(plan: SemanticBranchMutationPlan): string {
  return stableSerialize(plan.generation);
}

function executionEvidence(
  outcome: SemanticBranchExecutionOutcome,
  status: "succeeded" | "failed",
  branch?: Pick<GitHubBranch, "name" | "sha">,
  failure?: SemanticBranchExecutionFailureEvidence,
): SemanticBranchExecutionEvidence {
  return Object.freeze({
    version: SEMANTIC_BRANCH_EXECUTOR_CONTRACT_VERSION,
    planVersion: SEMANTIC_BRANCH_MUTATION_PLAN_VERSION,
    outcome,
    effects: Object.freeze([{ kind: "CREATE_BRANCH" as const, status }]),
    ...(branch === undefined ? {} : { branch: Object.freeze({ name: branch.name, sha: branch.sha }) }),
    ...(failure === undefined ? {} : { failure: Object.freeze(failure) }),
  });
}

function validateExecutionRequest(request: SemanticBranchExecutionRequest): void {
  if (!isRecord(request)) {
    throw new SemanticBranchExecutorError(
      "SEMANTIC_BRANCH_EXECUTION_REQUEST_INVALID",
      "Semantic branch execution request must be an object.",
    );
  }
  ...
  if (Object.keys(request).some((key) => !allowed.has(key))) {
    throw new SemanticBranchExecutorError(
      "SEMANTIC_BRANCH_EXECUTION_REQUEST_INVALID",
      "Semantic branch execution request contains an unsupported property.",
    );
  }
  if (request.version !== SEMANTIC_BRANCH_EXECUTOR_CONTRACT_VERSION) {
    throw new SemanticBranchExecutorError(
      "SEMANTIC_BRANCH_EXECUTION_REQUEST_INVALID",
      "Semantic branch execution request version is unsupported.",
      [
        diagnostic(
          "SEMANTIC_BRANCH_EXECUTION_REQUEST_VERSION_INVALID",
          "$.version",
  ...
  if (request.selector !== undefined && (typeof request.selector !== "string" || request.selector.length === 0)) {
    throw new SemanticBranchExecutorError(
      "SEMANTIC_BRANCH_EXECUTION_REQUEST_INVALID",
      "Semantic branch execution selector is invalid.",
      [diagnostic("SEMANTIC_BRANCH_EXECUTION_REQUEST_INVALID", "$.selector", "Selector must be a non-empty string.")],
    );
  }
}

function planInvalid(diagnostics: readonly SemanticBranchExecutionDiagnostic[]): SemanticBranchExecutorError {
  return new SemanticBranchExecutorError(
    "SEMANTIC_BRANCH_EXECUTION_PLAN_INVALID",
    "The Semantic Branch mutation plan is invalid and cannot be executed.",
    diagnostics,
  );
}

function revalidationFailed(diagnostics: readonly SemanticBranchExecutionDiagnostic[]): SemanticBranchExecutorError {
  return new SemanticBranchExecutorError(
    "SEMANTIC_BRANCH_EXECUTION_REVALIDATION_FAILED",
...
export class SemanticBranchExecutor implements SemanticBranchExecutionPort {
  ...
  async execute(request: SemanticBranchExecutionRequest): Promise<SemanticBranchExecutionResult> {
    validateExecutionRequest(request);
    const planResult = validateSemanticBranchMutationPlan(request.plan);
    if (!planResult.valid || planResult.plan === undefined) throw planInvalid(planResult.violations);
    ...
    if (planGeneration(plan) !== effectiveGeneration(effective)) {
      throw new SemanticBranchExecutorError(
        ...
        [
          diagnostic(
            "SEMANTIC_BRANCH_GOVERNANCE_GENERATION_MISMATCH",
            "$.generation",
    ...
    if (request.input !== undefined) {
      ...
      if (serializeSemanticBranchMutationPlan(replanned.plan) !== serializeSemanticBranchMutationPlan(plan)) {
        throw new SemanticBranchExecutorError(
          ...
          [diagnostic("SEMANTIC_BRANCH_PLAN_MISMATCH", "$.plan", "Versioned plan differs from the current Core plan.")],
        );
      }
      ...
      if (serializeSemanticBranchMutationPlan(replanned.plan) !== serializeSemanticBranchMutationPlan(plan)) {
        throw new SemanticBranchExecutorError(
          ...
          [
            diagnostic(
              "SEMANTIC_BRANCH_PLAN_ARTIFACT_MISMATCH",
              "$.artifact",
    ...
    if (targetPrecondition === undefined) {
      throw planInvalid([
        diagnostic(
          "SEMANTIC_BRANCH_TARGET_PRECONDITION_MISSING",
          "$.preconditions",
          "Target absence is required before creation.",
        ),
      ]);
    }
    let source: GitHubBranch | undefined;
    let target: GitHubBranch | undefined;
    try {
      ...
      throw new SemanticBranchExecutorError(
        "SEMANTIC_BRANCH_EXECUTION_READ_FAILED",
        "Unable to establish current branch state before mutation.",
        [
          diagnostic(
            "SEMANTIC_BRANCH_TARGET_READ_FAILED",
            "$.preconditions",
            "Current branch state could not be established.",
          ),
        ],
      );
    }
    if (source === undefined) {
      throw new SemanticBranchExecutorError(
        "SEMANTIC_BRANCH_EXECUTION_PRECONDITION_FAILED",
        "The planned source branch does not exist.",
        [diagnostic("SEMANTIC_BRANCH_SOURCE_ABSENT", "$.desired.source", "Source ref precondition is not satisfied.")],
      );
    }
    if (target !== undefined) {
      throw new SemanticBranchExecutorError(
        "SEMANTIC_BRANCH_EXECUTION_PRECONDITION_FAILED",
        "A branch already exists for the planned target name.",
        [
          diagnostic(
            "SEMANTIC_BRANCH_TARGET_EXISTS",
            "$.preconditions",
            "Target-absence precondition is not satisfied.",
          ),
        ],
      );
    }
    ...
    try {
      ...
      throw new SemanticBranchExecutorError(
        ...
        [diagnostic("SEMANTIC_BRANCH_EFFECT_FAILED", "$.effects[0]", "CREATE_BRANCH did not succeed.")],
        undefined,
    ...
    let after: GitHubBranch | undefined;
    try {
      after = await this.#adapter.findBranch(admittedPlan.desired.name);
    } catch {
      throw new SemanticBranchExecutorError(
        "SEMANTIC_BRANCH_EXECUTION_READ_FAILED",
        "Created branch could not be reread for postcondition verification.",
        [
          diagnostic(
            "SEMANTIC_BRANCH_POSTCONDITION_READ_FAILED",
            "$.projection",
            "Created branch state could not be reread.",
          ),
        ],
        undefined,
        executionEvidence("failed", "succeeded", created, {
    ...
    if (after === undefined || after.name !== admittedPlan.desired.name || after.sha !== source.sha) {
      throw new SemanticBranchExecutorError(
        "SEMANTIC_BRANCH_EXECUTION_PROJECTION_VERIFICATION_FAILED",
        "Observed branch does not satisfy the desired Semantic Branch projection.",
        [
          diagnostic(
            "SEMANTIC_BRANCH_PROJECTION_MISMATCH",
            "$.projection",
            "Observed branch differs from the desired projection.",
          ),
        ],
        undefined,
        executionEvidence("failed", "succeeded", after ?? created, {
