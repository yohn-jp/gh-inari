...
function assertSupportedEditMetadata(
  domain: GovernedArtifactDomain,
  metadata: ArtifactInputMetadata,
  remote: GitHubIssue | GitHubPullRequest,
): void {
  ...
  for (const key of keys) {
    if (domain === "pr" && key === "head") {
      ...
      throw new RemediationError(
        ...
        "$.metadata.head",
        { current: pullRequest.head, ...(requested === undefined ? {} : { requested }) },
      );
    }
    if (supported.has(key)) continue;
    throw new RemediationError(
      "SEMANTIC_PATCH_UNSUPPORTED",
      `Metadata "${key}" is not supported by ${domain === "issue" ? "issue" : "pull request"} edit.`,
      `$.metadata.${key}`,
      { metadata: key },
    );
  }
