...
export const SEMANTIC_PULL_REQUEST_PROJECTION_VERSION = "1" as const;
...
export type GitHubPullRequestProjectionCapability =
  (typeof GITHUB_PULL_REQUEST_PROJECTION_CAPABILITIES)[keyof typeof GITHUB_PULL_REQUEST_PROJECTION_CAPABILITIES];
...
export interface DesiredPullRequestProjection {
  ...
  readonly relations: Readonly<{ readonly implements: DesiredPullRequestRelationProjection }>;
...
function addViolation(
  ...
  code: SemanticPullRequestProjectionViolationCode,
  path: string,
  message: string,
): void {
...
function unknownProperties(
  ...
  path: string,
  violations: SemanticPullRequestProjectionViolation[],
  code: SemanticPullRequestProjectionViolationCode = "PROJECTION_INPUT_UNKNOWN_PROPERTY",
): void {
...
function invalidResult(
  violations: readonly SemanticPullRequestProjectionViolation[],
): SemanticPullRequestProjectionResult {
...
function normalizeReferences(
  ...
  if (!Array.isArray(value)) {
    addViolation(
      ...
      "SEMANTIC_ARTIFACT_VALUE_INVALID",
      path,
      "implements must be an array of IssueReference values.",
    );
    return undefined;
  }
...
interface ValidatedSemanticPullRequest {
  readonly artifact: SemanticArtifact;
  readonly title: string;
  readonly head: string;
  readonly base: string;
...
interface SemanticArtifactValidationResult {
  readonly artifact?: ValidatedSemanticPullRequest;
  readonly violations: readonly SemanticPullRequestProjectionViolation[];
}
...
function validateSemanticArtifact(input: unknown): SemanticArtifactValidationResult {
  ...
  if (input.kind !== "pull_request")
    addViolation(
      ...
      "SEMANTIC_ARTIFACT_INCOMPATIBLE",
      "$.artifact.kind",
      "A pull_request Semantic Artifact is required.",
    );
  ...
  return {
    artifact: {
      ...
      base,
      references,
      metadata: metadata as DesiredPullRequestMetadataProjection,
      fields,
      provenance,
    },
    violations: [],
...
function relationLabel(reference: IssueReference, provenance: ArtifactContractProvenance): string | undefined {
  const repository = provenance.repository;
  if (
    reference.repositoryHost.toLocaleLowerCase("en-US") === repository.host.toLocaleLowerCase("en-US") &&
    repository.repositoryId !== undefined &&
    reference.repositoryId === repository.repositoryId
  ) {
    return `#${reference.number}`;
  }
  if (reference.repository !== undefined) return `${reference.repository}#${reference.number}`;
  return undefined;
}

function relationBodyLines(
  ...
  references.forEach((reference, index) => {
    ...
    if (label === undefined) {
      addViolation(
        violations,
        "RELATION_UNREPRESENTABLE",
        `$.artifact.values.implements[${index}]`,
        "The IssueReference has no repository locator for a GitHub closing-reference projection.",
      );
      return;
    }
...
function relationFallbackMarker(references: readonly IssueReference[]): string {
...
function renderCanonicalBody(
  ...
  relationRepresentation: SemanticPullRequestProjectionRepresentation,
  references: readonly IssueReference[],
  provenance: ArtifactContractProvenance,
  violations: SemanticPullRequestProjectionViolation[],
): string | undefined {
  ...
  if (relationRepresentation === "recognized-convention") {
    ...
    if (lines !== undefined && lines.length > 0) blocks.push(lines.join("\n"));
...
function buildProjection(input: unknown, secondCapabilities?: unknown): SemanticPullRequestProjectionResult {
  const violations: SemanticPullRequestProjectionViolation[] = [];
  ...
  let representation: SemanticPullRequestProjectionRepresentation = "none";
  if (artifact.references.length > 0) {
    if (hasCapability(capabilities, "native")) representation = "native";
    else if (hasCapability(capabilities, "recognized")) representation = "recognized-convention";
    else if (hasCapability(capabilities, "fallback")) representation = "body-fallback";
    else {
      addViolation(
        violations,
        "RELATION_UNREPRESENTABLE",
        "$.capabilities",
        "No declared GitHub capability can represent the semantic implements relation.",
      );
    }
  }
  ...
  const desired: DesiredPullRequestProjection = {
...
export function tryProjectSemanticPullRequest(
  input: unknown,
...
export function projectSemanticPullRequest(input: unknown, capabilities?: unknown): DesiredPullRequestProjection {
  const result = tryProjectSemanticPullRequest(input, capabilities);
  if (!result.valid || result.projection === undefined) throw new SemanticPullRequestProjectionError(result.violations);
...
export function tryPlanSemanticPullRequest(
  input: unknown,
  capabilities?: unknown,
): SemanticPullRequestMutationPlanResult {
  ...
  const plan: SemanticPullRequestMutationPlan = {
...
export function planSemanticPullRequest(input: unknown, capabilities?: unknown): SemanticPullRequestMutationPlan {
  const result = tryPlanSemanticPullRequest(input, capabilities);
  if (!result.valid || result.plan === undefined) throw new SemanticPullRequestProjectionError(result.violations);
  return result.plan;
}
