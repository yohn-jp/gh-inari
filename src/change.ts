...
function projectionCommitSha(value: unknown, path: string, diagnostics: ChangeDiagnostic[]): string | undefined {
...
function parseChangeBranchEvidence(
  ...
  if (hasOwn(value, "sha")) sha = projectionCommitSha(value.sha, `${path}.sha`, diagnostics);
  ...
  if (diagnostics.length !== before || name === undefined) return undefined;
  return {
    name,
    ...(sha === undefined ? {} : { sha }),
...
function semanticPlanDiagnosticPath(path: string, violationPath: string): string {
  if (violationPath === "$" || violationPath.length === 0) return path;
...
function validateSemanticPullRequestPlan(
  input: unknown,
  path: string,
  diagnostics: ChangeDiagnostic[],
): SemanticPullRequestMutationPlan | undefined {
 ...
 */
function validateSemanticPlanIssuanceCapabilities(
  ...
  if (plan.desired.metadata.draft === false) {
    addDiagnostic(
      diagnostics,
      "CHANGE_INVALID_PLAN",
      `${path}.desired.metadata.draft`,
      "A Change issuance Semantic PR plan must remain Draft.",
    );
  }
  for (const key of ["milestone", "reviewers"] as const) {
    if (Object.prototype.hasOwnProperty.call(plan.desired.metadata, key)) {
      addDiagnostic(
        diagnostics,
        "CHANGE_INVALID_PLAN",
        `${path}.desired.metadata.${key}`,
        `The Change issuer cannot apply Semantic PR metadata "${key}" during issuance.`,
      );
    }
  }
  if (plan.desired.relations.implements.representation === "native") {
    addDiagnostic(
      diagnostics,
      "CHANGE_INVALID_PLAN",
      `${path}.desired.relations.implements`,
      "The Change issuer cannot apply a native Semantic PR implements relation during issuance.",
    );
  }
}
      ...
      if (target.semanticPullRequestPlan !== undefined) {
        ...
        if (desired.head !== target.branch) {
          addDiagnostic(
            diagnostics,
            "CHANGE_PROVENANCE_BRANCH_MISMATCH",
            "$.target.semanticPullRequestPlan.desired.head",
            "Semantic PR plan head must match the canonical Change branch.",
          );
        }
        if (desired.base !== target.baseBranch) {
          addDiagnostic(
            diagnostics,
            "CHANGE_PROVENANCE_BASE_MISMATCH",
            "$.target.semanticPullRequestPlan.desired.base",
            "Semantic PR plan base must match the canonical Change base branch.",
          );
        }
        validateSemanticPlanIssuanceCapabilities(
        ...
        if (
          repository.host.toLocaleLowerCase("en-US") !== change.identity.repositoryHost ||
          repository.repositoryId !== change.identity.repositoryId
        ) {
          addDiagnostic(
            diagnostics,
            "CHANGE_PROVENANCE_IDENTITY_MISMATCH",
            "$.target.semanticPullRequestPlan.provenance.repository",
            "Semantic PR plan repository identity must match the Change identity.",
          );
        }
      }
    }
 ...
 */
function initialChangePullRequestCompatibilityPayload(rootIssue: number): {
  readonly title: string;
  readonly body: string;
} {
  return {
...
function validateEffectCommitSha(value: unknown, path: string, diagnostics: ChangeDiagnostic[]): string | undefined {
  if (typeof value !== "string" || value.length !== MAX_CHANGE_COMMIT_SHA_LENGTH || !/^[0-9a-f]{40}$/iu.test(value)) {
    ...
    if (semanticPullRequestPlan !== undefined) {
      if (semanticPullRequestPlan.desired.head !== branch) {
        addDiagnostic(
          diagnostics,
          "CHANGE_INVALID_PLAN",
          `${path}.semanticPullRequestPlan.desired.head`,
          "Semantic PR plan head must match the effect branch.",
        );
      }
      if (semanticPullRequestPlan.desired.base !== baseBranch) {
        addDiagnostic(
          diagnostics,
          "CHANGE_INVALID_PLAN",
          `${path}.semanticPullRequestPlan.desired.base`,
          "Semantic PR plan base must match the effect base branch.",
        );
      }
      if (semanticPullRequestPlan.desired.title !== title || semanticPullRequestPlan.desired.body !== body) {
        addDiagnostic(
          diagnostics,
          "CHANGE_INVALID_PLAN",
          `${path}.semanticPullRequestPlan.desired`,
          "Effect title and body must be the Semantic PR plan desired projection.",
        );
      }
...
export function validateChangeEffectSuccessEvidence(
  input: unknown,
  effect?: ChangeEffect,
  path = "$",
): ChangeEffectSuccessEvidenceValidationResult {
  const diagnostics: ChangeDiagnostic[] = [];
  if (!isRecord(input)) {
  ...
  if (kind === "CREATE_BRANCH") {
    ...
    rejectEffectProperties(
      input,
      new Set(["kind", "branch", "baseBranch", "rootIssue", "pullRequest"]),
      path,
      diagnostics,
    );
    const branch = requiredEffectBranch(input, "branch", path, MAX_CHANGE_BRANCH_LENGTH, diagnostics);
    ...
    if (hasOwn(input, "expectedCommitSha")) {
      ...
      if (branch !== undefined) evidence = { kind, branch };
    }
  }
...
function effectSuccessEvidenceMatches(effect: ChangeEffect, evidence: ChangeEffectSuccessEvidence): boolean {
  if (effect.kind !== evidence.kind) return false;
  switch (effect.kind) {
    case "CREATE_BRANCH":
      return (
...
function buildChangeIssuancePlan(
  ...
  semanticPullRequestPlan?: SemanticPullRequestMutationPlan,
): ChangeIssuancePlan {
  if (
...
export function planChangeIssuance(input: unknown): ChangeIssuancePlan {
  ...
  if (isRecord(input) && hasOwn(input, "semanticPullRequestPlan")) {
    const planDiagnostics: ChangeDiagnostic[] = [];
    semanticPullRequestPlan = validateSemanticPullRequestPlan(
      input.semanticPullRequestPlan,
      "$.semanticPullRequestPlan",
      planDiagnostics,
...
export function validateChangeIssuancePlan(input: unknown): ChangeIssuancePlanValidationResult {
  ...
  if (mode === "create") {
    ...
    const semanticPullRequestPlan = effects.find(
      (effect): effect is Extract<ChangeEffect, { readonly kind: "CREATE_PULL_REQUEST" }> =>
...
function validateChangeIssuanceEffectAttempts(
  ...
  for (let index = 0; index < input.length && diagnostics.length < MAX_CHANGE_DIAGNOSTICS; index += 1) {
    ...
    const status = CHANGE_ISSUANCE_EFFECT_STATUSES.includes(attempt.status as ChangeIssuanceEffectStatus)
      ? (attempt.status as ChangeIssuanceEffectStatus)
      : undefined;
    let evidence: ChangeEffectSuccessEvidence | undefined;
    if (hasOwn(attempt, "evidence") && effectResult.effect !== undefined) {
      const evidenceResult = validateChangeEffectSuccessEvidence(
        attempt.evidence,
        effectResult.effect,
        `${path}[${index}].evidence`,
      );
      diagnostics.push(
        ...evidenceResult.diagnostics.slice(0, Math.max(0, MAX_CHANGE_DIAGNOSTICS - diagnostics.length)),
      );
    ...
    if (status === "failed" && hasOwn(attempt, "evidence")) {
      addDiagnostic(
        ...
        "CHANGE_INVALID_PLAN",
        `${path}[${index}].evidence`,
        "A failed effect attempt cannot contain success evidence.",
      );
    }
...
function validateCompensationPlanEffects(
  ...
  if (
    ...
    addRecoverySemanticDiagnostic(
      diagnostics,
      path,
      "Compensation must contain exactly one SHA-conditional delete effect for the canonical branch.",
    );
  }
}
...
function validateFailureProjectionShape(
  ...
  const canonicalBranchCandidates = projection.candidates.branches.filter(
    (candidate) => candidate.candidate.name === expectedBranch,
  );
  ...
  if (projection.status !== "partial" && !confirmedAbsent) {
    addRecoverySemanticDiagnostic(
      diagnostics,
      path,
      "Branch compensation requires a confirmed branch-only projection or confirmed branch absence.",
    );
  }
  ...
  if (!confirmedAbsent) {
    ...
    if (expectedCommitSha === undefined || currentCommitSha !== expectedCommitSha) {
      addRecoverySemanticDiagnostic(
        diagnostics,
        `${path}.candidates.branches`,
        "The current canonical branch generation does not match the created generation.",
      );
    }
  }
...
function buildChangeIssuanceCompensationPlan(
  ...
  const createdCommitSha = validateCreateIssuanceFailureSemantics(
    parsed.issuance,
    parsed.attemptedEffects,
    parsed.failure,
    diagnostics,
  );
  ...
  if (diagnostics.length > 0 || createdCommitSha === undefined) return undefined;
...
export function validateChangeIssuanceCompensationPlan(input: unknown): ChangeIssuanceCompensationPlanValidationResult {
  ...
  validateCompensationPlanEffects(
    effects,
    issuance.verification.canonicalBranch,
    createdCommitSha,
    "$.effects",
