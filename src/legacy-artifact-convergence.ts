...
export const LEGACY_ARTIFACT_CONVERGENCE_VERSION = "1" as const;
export const MAX_LEGACY_CONVERGENCE_DIAGNOSTICS = 32 as const;

export type LegacyArtifactCompatibilitySource = "candidate" | "document";

export type LegacyArtifactConvergenceCode =
  | "LEGACY_INPUT_INVALID"
  | "LEGACY_PROVENANCE_MISSING"
  | "LEGACY_UNKNOWN_METADATA"
  | "LEGACY_RELATION_UNSUPPORTED"
  | "LEGACY_RELATION_INVALID"
  | "LEGACY_SEMANTIC_CONFLICT"
  | "LEGACY_LINKED_ISSUE_INVALID"
  | "LEGACY_LINKED_ISSUE_UNRESOLVED"
  | "LEGACY_TITLE_INVALID"
  | "LEGACY_BRANCH_INVALID"
...
function diagnostic(
  code: LegacyArtifactConvergenceCode,
  path: string,
  message: string,
...
function sortDiagnostics(
  diagnostics: readonly LegacyArtifactConvergenceDiagnostic[],
): readonly LegacyArtifactConvergenceDiagnostic[] {
  return [...diagnostics]
...
function cloneJson<T>(value: T): T {
  ...
  if (isRecord(value)) {
    ...
    for (const key of Object.keys(value).sort(compareStrings)) output[key] = cloneJson(value[key]);
    return output as T;
  }
  return value;
}

function sourceFromInput(input: ArtifactCandidate | ArtifactInputDocument): LegacyArtifactCompatibilitySource {
  return "source" in input ? "candidate" : "document";
}

function candidateParts(input: ArtifactCandidate | ArtifactInputDocument): {
...
function isEffectiveArtifactContract(value: unknown): value is EffectiveArtifactContract {
  return (
    isRecord(value) &&
    value.version === "1" &&
...
function referencesEqual(left: readonly IssueReference[], right: readonly IssueReference[]): boolean {
...
function normalizeReferences(
  ...
  const diagnostics: LegacyArtifactConvergenceDiagnostic[] = [];
  ...
  input.forEach((entry, index) => {
    ...
    if (seen.has(key)) {
      diagnostics.push(
        diagnostic("LEGACY_RELATION_INVALID", `${path}[${index}]`, `Duplicate IssueReference "${key}".`),
      );
      return;
    }
    seen.add(key);
...
function linkedIssueReferenceFromText(
  value: unknown,
  repository: LegacyLinkedIssueRepository | undefined,
  path: string,
): { readonly reference?: IssueReference; readonly diagnostics: readonly LegacyArtifactConvergenceDiagnostic[] } {
  if (isRecord(value)) {
    ...
    return {
      diagnostics: [
        diagnostic(
          "LEGACY_LINKED_ISSUE_INVALID",
          path,
          "linkedIssue must be a complete closing reference; arbitrary prose is not semantic input.",
        ),
      ],
    };
  }
  const locator = match[1] as string;
    ...
    return {
      diagnostics: [
        diagnostic(
          "LEGACY_LINKED_ISSUE_UNRESOLVED",
          path,
          "A local linked Issue requires explicit repository identity before it can become implements.",
        ),
      ],
    };
  }
    ...
    return {
      diagnostics: [
        diagnostic(
          "LEGACY_LINKED_ISSUE_UNRESOLVED",
          path,
          "Cross-repository linked Issue identity does not match the supplied repository authority.",
        ),
      ],
    };
  }
...
function relationName(kind: ArtifactContractKind): "dependsOn" | "implements" | undefined {
  if (kind === "issue") return "dependsOn";
  if (kind === "pull_request") return "implements";
  return undefined;
}

function declaredProperty(effective: EffectiveArtifactContract, name: string): PropertyValueDeclaration | undefined {
  return effective.properties[name];
}

function mapLegacyInput(
  effective: EffectiveArtifactContract,
  input: ArtifactCandidate | ArtifactInputDocument,
  options: LegacyArtifactConvergenceOptions,
  ...
  const semantic: RecordValue = cloneJson(parts.fields);
  ...
  for (const field of effective.fields ?? []) {
    if (field.primitive !== "choice" || !hasOwn(semantic, field.id) || Array.isArray(semantic[field.id])) continue;
    semantic[field.id] = [semantic[field.id]];
  }
  const relation = relationName(effective.kind);

  const metadataKeys: readonly (keyof ArtifactInputMetadata)[] = [
    "title",
    "labels",
    "assignees",
    "head",
  ...
  for (const key of metadataKeys) {
    ...
    if (declaration === undefined || declaration.presence === "unused") {
      diagnostics.push(
        diagnostic("LEGACY_UNKNOWN_METADATA", `$.metadata.${key}`, `Legacy metadata "${key}" is not declared by Core.`),
      );
      continue;
    }
    if (hasOwn(semantic, key) && stableValue(semantic[key]) !== stableValue(value)) {
      diagnostics.push(
        diagnostic(
          "LEGACY_SEMANTIC_CONFLICT",
          `$.metadata.${key}`,
          `Legacy metadata and semantic value "${key}" conflict.`,
        ),
      );
      continue;
    }
  ...
  const sidecarValidation =
    effective.kind === "issue" ? validateIssueDependencies(parts.dependencies as unknown) : undefined;
  if (sidecarValidation !== undefined && !sidecarValidation.valid) {
    for (const violation of sidecarValidation.violations) {
      diagnostics.push(
        diagnostic("LEGACY_RELATION_INVALID", `$.dependencies${violation.path.slice(1)}`, violation.message),
      );
    }
  }
  ...
  if (
    ...
    diagnostics.push(
      diagnostic(
        "LEGACY_RELATION_UNSUPPORTED",
        "$.dependencies",
        "Issue dependency sidecar is supported only as an Issue dependsOn compatibility ingress.",
      ),
    );
  }
    ...
    if (relation !== "dependsOn" || declaredProperty(effective, "dependsOn")?.presence === "unused") {
      diagnostics.push(
        diagnostic(
          ...
          "Legacy dependencies have no declared semantic dependsOn relation.",
        ),
      );
      ...
      if (parsed.references !== undefined && !referencesEqual(parsed.references, sidecarBlockedBy)) {
        diagnostics.push(
          diagnostic(
            "LEGACY_SEMANTIC_CONFLICT",
            "$.dependencies.blockedBy",
            "Legacy dependency evidence conflicts with semantic dependsOn.",
          ),
        );
      }
    } else {
  ...
  if (relation === "implements") {
    ...
    let linked: readonly IssueReference[] | undefined = explicit;
    if (linkedValue !== undefined) {
      ...
      if (linked === undefined) linked = textReferences;
      else if (textReferences !== undefined && !referencesEqual(textReferences, linked)) {
        diagnostics.push(
          diagnostic(
            "LEGACY_SEMANTIC_CONFLICT",
            "$.linkedIssue",
            "Legacy linkedIssue conflicts with typed implements.",
          ),
        );
      }
    }
    if (linked !== undefined) {
      const parsed = hasOwn(semantic, "implements")
        ...
        : undefined;
      if (parsed !== undefined) diagnostics.push(...parsed.diagnostics);
      if (parsed?.references !== undefined && !referencesEqual(parsed.references, linked)) {
        diagnostics.push(
          diagnostic(
            "LEGACY_SEMANTIC_CONFLICT",
            "$.implements",
            "Legacy linkedIssue evidence conflicts with semantic implements.",
          ),
        );
    ...
    if (title === options.nativeTitlePrefix.trim()) {
      diagnostics.push(
        diagnostic(
          "LEGACY_TITLE_INVALID",
          "$.metadata.title",
          "Legacy title must contain content beyond the native prefix.",
        ),
      );
    }
  }
  if (options.branch !== undefined) {
    ...
    if (errors.length > 0)
      diagnostics.push(diagnostic("LEGACY_BRANCH_INVALID", "$.metadata.head", errors[0] as string));
  }
...
export function convergeLegacyArtifactInput(
...
function fieldConstraints(field: CanonicalField): FieldContentConstraints | undefined {
  ...
  const values =
    field.type === "enum"
      ...
      : field.type === "array"
        ...
        : undefined;
  if (constraints === undefined && values === undefined && field.type !== "checklist") return undefined;
  return {
...
function fieldDeclaration(field: CanonicalField): FieldDeclaration {
  const presence = field.required === "required" ? "required" : "optional";
  const primitive =
    field.type === "string" ? "text" : field.type === "enum" || field.type === "array" ? "choice" : "checklist";
  return {
    id: field.id,
    primitive,
    presence,
...
function oldProvenance(provenance: ContractProvenance | undefined): ArtifactContractProvenance | undefined {
 ...
 */
export function artifactContractFromLegacyCanonical(contract: CanonicalContract): ArtifactContract {
  const linkedFieldIds = new Set(
    contract.supplementalConstraints.fields.filter((entry) => entry.linkedIssue === true).map((entry) => entry.fieldId),
  );
...
export function compileLegacyEffectiveArtifactContract(contract: CanonicalContract): EffectiveArtifactContract {
...
export function tryMaterializeLegacyArtifact(
  contract: CanonicalContract | EffectiveArtifactContract,
  input: ArtifactCandidate | ArtifactInputDocument,
  options: LegacyArtifactConvergenceOptions = {},
): LegacySemanticArtifactMaterializationResult {
  let effectiveContract: EffectiveArtifactContract;
  if (isEffectiveArtifactContract(contract)) effectiveContract = contract;
  else {
    try {
      effectiveContract = compileLegacyEffectiveArtifactContract(contract);
    } catch (error: unknown) {
      const code =
        error instanceof Error && error.message.includes("provenance")
          ? "LEGACY_PROVENANCE_MISSING"
          : "LEGACY_CONTRACT_UNSUPPORTED";
      return {
        valid: false,
        diagnostics: [
          diagnostic(
            code,
            code === "LEGACY_PROVENANCE_MISSING" ? "$.provenance" : "$.contract",
            error instanceof Error ? error.message : "Legacy contract cannot be converged.",
          ),
        ],
      };
    }
...
export function materializeLegacyArtifact(
  contract: CanonicalContract | EffectiveArtifactContract,
  input: ArtifactCandidate | ArtifactInputDocument,
  options: LegacyArtifactConvergenceOptions = {},
): SemanticArtifact {
  ...
  if (!result.valid || result.artifact === undefined) {
    throw new SemanticArtifactMaterializationError(
...
export function validateLegacyBranchProjection(branch: string): LegacyArtifactConvergenceResult {
  const errors = validateBranchName(branch);
  return {
    version: LEGACY_ARTIFACT_CONVERGENCE_VERSION,
    valid: errors.length === 0,
    ...(errors.length === 0 ? {} : { compatibility: { metadata: {}, source: "candidate" as const } }),
