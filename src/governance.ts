 ...
 */
export async function compileLocalBranchGovernance(
  root: string,
): Promise<EffectivePullRequestBranchGovernance | undefined> {
  const selectedPolicy = await resolveLocalPolicyPath(root, undefined);
  if (selectedPolicy === undefined) return undefined;
...
async function compileRepositoryGovernedContractFromSource(
  ...
  let branchGovernance: EffectivePullRequestBranchGovernance | undefined;
