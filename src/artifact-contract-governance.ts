...
function isPullRequestCanonPath(value: string): boolean {
  if (!value.endsWith(".json")) return false;
  return (
    value === ".github/inari/canon/pull-request.json" ||
...
function candidateId(path: string): string {
  ...
  if (base === "pull-request" || base === "pull_request") return "default";
  return base;
}

function canonCandidates(tree: readonly RepositoryTreeEntry[]): readonly CanonCandidate[] {
  return tree
...
function selectCandidate(
  candidates: readonly CanonCandidate[],
  ...
  if (candidates.length === 0) {
    throw new ArtifactContractResolutionError(
      "ARTIFACT_CONTRACT_NOT_FOUND",
      "$.source",
      `No pull_request Artifact Contract Canon was found for ${context.nameWithOwner} at ref "${ref}".`,
      { repository: context.nameWithOwner, ref, attemptedPaths: ARTIFACT_CONTRACT_CANON_PATHS },
    );
  }
  ...
  if (matches.length === 1) return matches[0] as CanonCandidate;
  if (matches.length > 1) {
    throw new ArtifactContractResolutionError(
      "ARTIFACT_CONTRACT_SELECTOR_AMBIGUOUS",
      "$.selector",
      `Artifact Contract selector "${requested ?? "default"}" matches multiple Canon sources.`,
      { selector: requested ?? "default", matches: matches.map((candidate) => candidate.entry.path) },
    );
  }
  throw new ArtifactContractResolutionError(
    "ARTIFACT_CONTRACT_NOT_FOUND",
    "$.selector",
    `No pull_request Artifact Contract Canon matches selector "${requested ?? "default"}".`,
    { selector: requested ?? "default", candidates: candidates.map((candidate) => candidate.entry.path) },
  );
}
