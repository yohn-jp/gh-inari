...
export type ArtifactDiagnosticReason =
  "accepted" | "required" | "type" | "constraint" | "conflict" | "unsupported" | "unrecoverable";

/** Stable detail code for field-level and template-level projections. */
export type ArtifactDiagnosticDetailCode =
 ...
 */
export type ArtifactDiagnosticEvidence =
  | StringFieldEvidence
  | NumberFieldEvidence
  | BooleanFieldEvidence
  | NullFieldEvidence
  | UndefinedFieldEvidence
  | ArrayFieldEvidence
  | ObjectFieldEvidence;
...
export const MAX_EVIDENCE_LENGTH = 240 as const;
export const MAX_EVIDENCE_ITEMS = 8 as const;
export const MAX_EVIDENCE_COLLECTION_LENGTH = 128 as const;
...
const RECOVERY_ACTIONS: readonly ArtifactDiagnosticRecoveryAction[] = [
  "provide",
  "replace",
  "resolve-conflict",
  "select-template",
...
const EVIDENCE_TYPES: readonly ArtifactEvidenceType[] = [
  "string",
  "number",
  "boolean",
  "null",
...
const PROHIBITED_FIELD_SEGMENT =
  /^(?:rawbody|renderedbody|fullartifact|artifact|body|payload|document|accesstoken|privatekey|sessionsecret|authorization|cookie|credential|password|secret|token)$/i;
...
const DETAIL_DEFINITIONS: Readonly<Record<ArtifactDiagnosticDetailCode, DiagnosticDefinition>> = {
 ...
 */
export function createFieldEvidence(field: string, value: unknown): ArtifactDiagnosticEvidence {
  const normalizedField = assertFieldPath(field);
  if (value === null) return { field: normalizedField, type: "null" };
  if (value === undefined) return { field: normalizedField, type: "undefined" };
  ...
  if (typeof value === "boolean") return { field: normalizedField, type: "boolean", value };
  if (typeof value === "number") {
    if (!Number.isFinite(value)) throw new TypeError("Non-finite numbers are not valid diagnostic evidence.");
...
export function createArtifactDiagnostic(input: ArtifactDiagnosticInput): ArtifactDiagnostic {
  ...
  if (definition === undefined) throw new TypeError(`Unsupported artifact diagnostic detail code: ${detailCode}.`);
...
function canonicalizeDiagnostic(diagnostic: ArtifactDiagnostic): ArtifactDiagnostic {
  ...
  if (definition.reason !== diagnostic.reason) throw new TypeError("Artifact diagnostic has an invalid reason.");
  return {
    version: ARTIFACT_DIAGNOSTIC_VERSION,
    state: definition.state,
    code: definition.code,
...
function normalizeEvidence(value: unknown): ArtifactDiagnosticEvidence {
  if (!isRecord(value)) {
  ...
  if (typeof value.field !== "string") throw new TypeError("Diagnostic evidence requires a field identity.");
  const field = assertFieldPath(value.field);
  if (typeof value.type !== "string" || !EVIDENCE_TYPES.includes(value.type as ArtifactEvidenceType)) {
    throw new TypeError("Diagnostic evidence has an unsupported type.");
  }
  switch (value.type as ArtifactEvidenceType) {
    case "string":
    ...
    case "boolean":
      if (typeof value.value !== "boolean") throw new TypeError("Boolean evidence has an invalid value.");
    ...
    case "array": {
      ...
      if (
        ...
        !value.itemTypes.every(
          (type) => typeof type === "string" && EVIDENCE_TYPES.includes(type as ArtifactEvidenceType),
        )
      ) {
      ...
      return {
        ...
        itemTypes: [...new Set(value.itemTypes as ArtifactEvidenceType[])].sort(compareText),
...
function compareRecovery(left: ArtifactDiagnosticRecovery, right: ArtifactDiagnosticRecovery): number {
  return compareText(
    [left.action, left.path ?? "", left.hint ?? ""].join("\u0000"),
    [right.action, right.path ?? "", right.hint ?? ""].join("\u0000"),
  );
}
...
function assertFieldPath(value: string): string {
...
function valueType(value: unknown): ArtifactEvidenceType {
      ...
      (Array.isArray(value.recovery) &&
        value.recovery.every(
          (entry) =>
            ...
            RECOVERY_ACTIONS.includes(entry.action as ArtifactDiagnosticRecoveryAction) &&
...
function isEvidence(value: unknown): value is ArtifactDiagnosticEvidence {
  if (!isRecord(value)) return false;
  return (
    typeof value.field === "string" &&
    typeof value.type === "string" &&
    EVIDENCE_TYPES.includes(value.type as ArtifactEvidenceType)
  );
}
