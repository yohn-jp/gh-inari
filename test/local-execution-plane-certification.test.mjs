...
async function occupyPort(port, allowAlreadyOccupied = false) {
  const server = createServer();
  if (!allowAlreadyOccupied) {
...
async function startServer(group, cwd, env) {
  const child = spawn(process.execPath, [cliEntry, group, "serve", "--json"], {
    cwd,
    env,
...
async function startSupervisor(cwd, env) {
  const child = spawn(process.execPath, [cliEntry, "runtime", "supervise", "--json"], {
    cwd,
    env,
    stdio: ["ignore", "pipe", "pipe"],
  });
...
test(
  "#1122 certifies the packed installed CLI, actual local Admission, and Executor processes",
  { timeout: 180_000 },
  async () => {
    const packed = await installPackedCli();
    cliEntry = packed.entry;
    ...
    try {
      ...
      assert.equal(
        wrongAdmission.error,
        undefined,
        `Wrong Executor identity check timed out: ${wrongAdmission.stderr}`,
      );
      ...
      for (const [label, providerState] of [
        ...
        await writeFile(stateFile, JSON.stringify({ branchPresent: true, pullRequestState: "open" }));
        jsonOutput(
          command(["session", "close", "--json"], { cwd: workspace, env: denialEnv }),
          `${label} session close`,
        );
      }
      await writeFile(stateFile, JSON.stringify({ branchPresent: true, pullRequestState: "open" }));
      ...
      assert.equal(
        existsSync(discoveryPath(configHome, "executor")),
        false,
        "Supervisor left Executor discovery active",
      );
      assert.equal(
        existsSync(discoveryPath(configHome, "admission")),
        false,
      ...
      for (const server of servers.reverse()) await stopServer(server);
      for (const server of historicalPortOccupants.reverse().filter((value) => value !== undefined)) {
        server.close();
