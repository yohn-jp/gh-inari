...
Status: foundation for Source #1100 (Implementation #1111). Consumer migration is owned by #1112 (local Session/Admission/setup) and #1113 (Change/provider).
...
| `src/change.ts:1643` (canonical branch derivation)                                                   | `deriveBranchName` plus pattern-only `branchGovernance`                                                                              | P2 #1113                               |
| `src/github/change-state-projector.ts:136,226,281,741`                                               | `deriveBranchNamingFromIssueTitle`, `recognizeBranchNamingForIssue`, `branchBelongsToRootIssue`, `resolveRepositoryBranchGovernance` | P2 #1113                               |
...
| `src/integration-routing.ts:210,228,382,628`                                                         | `validateBranchName`, `recognizeBranchName`, `DEFAULT_BRANCH_NAME`                                                                   | retained (reserved routing)            |
| `src/implementation-contract.ts:813,824`                                                             | `validateBranchName` (reserved-namespace checks)                                                                                     | retained                               |
| `src/implementation-change-identity.ts:301,306`                                                      | `validateBranchName`, `recognizeBranchName`                                                                                          | retained (historical identity)         |
| `src/agent-authority/branch-advance.ts:222`, `capability.ts:115,119`, `capability-provenance.ts:348` | `validateBranchName`                                                                                                                 | retained (signed capability records)   |
| `src/legacy-artifact-convergence.ts:490,724`                                                         | `validateBranchName`                                                                                                                 | retained (historical artifacts)        |
| `src/github/git-data-capability.ts:432`                                                              | `validateBranchName`                                                                                                                 | retained                               |
